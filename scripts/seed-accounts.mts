/**
 * Puts the company's own accounts on the system, and gives the payments
...
async function main() {
  ...
  // payment in kwacha went into the cash tin; there is nowhere else for it
  // to have gone. A cash payment in DOLLARS did not go into the kwacha cash
  ...
  for (const payment of payments) {
    const accountId =
      payment.accountId ??
      (payment.method === "CASH" && payment.currency === "ZMW"
        ? byCode.get("CASH_OFFICE_ZMW")?.id
        : payment.method === "CASH" && payment.currency === "USD"
          ? /* AITRANSIT keeps a dollar tin as well as a kwacha one, so this
               guess is now safe where it was not for AITRANSIT. It is
               still only made for CASH — a bank transfer could have landed in
               either bank account and there is nothing in the record to say
               which. */
            byCode.get("CASH_OFFICE_USD")?.id
          : undefined);
