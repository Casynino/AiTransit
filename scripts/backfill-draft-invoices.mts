 ...
 * Lusaka arrived before that and carries no invoice at all — so Finance would open
...
const cargo = await prisma.shipment.findMany({
  where: {
    deletedAt: null,
    status: { in: ["RECEIVED_AT_ZAMBIA", "READY_FOR_PICKUP", "DELIVERED"] },
  },
  select: {
    id: true,
    trackingNumber: true,
...
console.log(
  `${cargo.length} consignments on or past the Lusaka floor.\n` +
