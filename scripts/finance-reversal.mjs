...
// A kwacha cost that has not been paid yet, and a kwacha account to pay it
// from — the currencies have to match or the action refuses, correctly.
/*
  An unpaid kwacha cost if there is one, so the run covers pay-then-reverse.
...
let target = await prisma.expense.findFirst({
  where: { status: "PENDING", currency: "ZMW" },
...
if (!target) throw new Error("No kwacha cost left to pay or reverse.");

const account = await prisma.companyAccount.findFirst({
  where: { active: true, currency: "ZMW" },
  select: { id: true, name: true },
});
if (!account) throw new Error("No active kwacha account.");

console.log(`\nworking on ${target.expenseNumber} — ${target.description}`);
console.log(`  ZMW ${money(target.amount)} against ${account.name}`);
console.log(`  balance before        ZMW ${money(await balance(account.id))}`);
...
await page.setCookie(...(await cookies("finance@aitransit.co.zm")));
...
console.log(`  balance after paying  ZMW ${money(await balance(account.id))}`);
...
console.log(`  balance after reverse ZMW ${money(await balance(account.id))}`);
console.log("\n  the register still holds both sides:");
for (const e of entries) {
  console.log(
    `    ${e.entryNumber}  ${e.direction.padEnd(3)}  ZMW ${money(e.amount).padStart(12)}  ${e.kind}${e.reversesId ? "  → cancels the line above" : ""}`
  );
}
