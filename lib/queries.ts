 ...
 *
 * Sums `creditedAmount`, never `amount`. A customer may hand over kwacha or
 ...
 *
 * "84 bills unpaid, K 25.4m" is a number nobody can act on. The first
 ...
 * it". A month where K 30m came in and K 29m went out looks identical to a
...
export async function corridorPosition() {
  const [inChina, inAir, onFloor, ready, flagged] = await Promise.all([
    ...
    prisma.shipment.count({
      where: { deletedAt: null, status: "RECEIVED_AT_ZAMBIA" },
    }),
    prisma.shipment.count({
...
export async function deskPulse(
  ...
  const [
    ...
    prisma.shipment.count({ where: { deletedAt: null, status: "RECEIVED_AT_ZAMBIA" } }),
    prisma.shipment.count({
      where: {
        deletedAt: null,
  ...
  return [
    ...
    {
      key: "zambia",
      desk: t(locale, "Lusaka floor"),
      href: "/app/inventory",
      headline: String(zambiaStanding),
      headlineLabel: t(locale, "on the floor"),
      detail: count(readyNotCollected, "cleared and waiting to be collected"),
      problem:
        zambiaAging > 0
          ? count(zambiaAging, "past the free storage window")
          : null,
      tone: "brand",
    },
    {
      ...
      // Kwacha lead. The owner reads this the way the till does; the dollar
      // figure is what the invoice says and lives on the finance desk's page.
      problem:
        owedUsd > 0
          ? rate
            ? `K ${Math.round(owedUsd * rate).toLocaleString("en-US")} ${t(locale, "outstanding")}`
...
export async function ownerAttention(
  ...
  const [china, floor, zambiaProblems, finance, support] = await Promise.all([
    chinaProblems(),
    floorSnapshot(),
    (async () => {
  ...
  const money = (usd: number) =>
    rate ? `K ${Math.round(usd * rate).toLocaleString("en-US")}` : `USD ${usd.toFixed(2)}`;
  ...
  return [
    ...
    {
      when: zambiaProblems.unfinishedBatches > 0,
      id: "zambia-unfinished",
      group: t(locale, "Lusaka floor"),
      severity: "warning" as const,
      label: count(zambiaProblems.unfinishedBatches, "landed batch(es) not finished"),
      detail: t(
        locale,
    ...
    {
      when: zambiaProblems.readyForPickup > 0,
      id: "zambia-ready",
      group: t(locale, "Lusaka floor"),
      severity: "info" as const,
      label: count(zambiaProblems.readyForPickup, "paid, not collected"),
    ...
    {
      when: zambiaProblems.openCases > 0,
      id: "cases",
      group: t(locale, "Cases"),
      severity: "critical" as const,
      label: count(zambiaProblems.openCases, "open case(s)"),
...
/** Counts the Lusaka warehouse cares about. */
export async function zambiaStats() {
  const [incoming, awaitingCheck, inWarehouse, readyForPickup, openExceptions] =
    await Promise.all([
      ...
      prisma.shipment.count({ where: { status: "RECEIVED_AT_ZAMBIA" } }),
    ...
    await Promise.all([
      ...
      prisma.shipment.count({
        where: {
          ...
          status: { in: ["RECEIVED_AT_ZAMBIA", "IN_TRANSIT"] },
        },
      }),
 ...
 *
 * Wrapped in React's request cache because the admin's home renders this
 * twice — once inside adminOverview's Promise.all and once inside the shared
...
export const executiveStats = cache(async function executiveStats() {
  ...
  const [
    ...
    prisma.shipment.count({ where: { status: "IN_TRANSIT" } }),
    prisma.shipment.count({
      where: { status: { in: ["RECEIVED_AT_ZAMBIA", "READY_FOR_PICKUP"] } },
    }),
    prisma.shipment.count({
...
/**
 * The Lusaka warehouse's inbound queue.
 *
...
export async function attentionItems(
  role: "ADMIN" | "CHINA_WAREHOUSE" | "ZAMBIA_WAREHOUSE" | "FINANCE",
  locale: Locale = "en"
): Promise<AttentionItem[]> {
  const sees = {
    exceptions: role !== "CHINA_WAREHOUSE",
    verification: role === "ZAMBIA_WAREHOUSE" || role === "ADMIN",
    money: role === "FINANCE" || role === "ADMIN",
    china: role === "CHINA_WAREHOUSE" || role === "ADMIN",
    collection: role === "ZAMBIA_WAREHOUSE" || role === "FINANCE" || role === "ADMIN",
  };
  ...
  for (const shipment of staleUnpaid) {
    ...
    items.push({
      ...
      detail: `${shipment.customer.name} (${shipment.customer.phone}) ${t(locale, "owes")} ${shipment.invoice?.currency ?? "ZMW"} ${outstanding.toLocaleString()}.`,
