...
/**
 * The admin's control layer: what the books say against what the world says.
 *
...
export type AccountStanding = {
   ...
   *
   * Not restated in kwacha, unlike every customer-facing figure in this app.
...
export async function reviewsFor(
  ...
  const rows = await prisma.recordReview.findMany({
...
export async function reviewHistory(target: ReviewTarget, targetId: string) {
  return prisma.recordReview.findMany({
...
/**
 * Every record a admin has disputed and nobody has closed.
 *
...
export async function openReviews(now = new Date()): Promise<OpenReview[]> {
  const newest = await prisma.recordReview.findMany({
  ...
  return newest
    .filter(
      (r) =>
        ...
        /* A flag is the loudest thing a admin can say about a record, and a
