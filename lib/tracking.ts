 ...
 * exactly like cargo sitting damaged on the shelf in Makeni, because the
...
> = {
  ...
  IN_TRANSIT: {
    UNDER_INVESTIGATION: {
      ...
      location: "Being traced between China and Lusaka",
    },
    COMPENSATION_IN_PROGRESS: {
      label: "Claim being settled",
...
export type PublicCharge = {
  ...
  paid: number;
  outstanding: number;
  /** Kwacha figures at the rate the invoice was raised at. */
   ...
   * kwacha figure was reached rather than being asked to trust it.
   *
   ...
   * free window closes, and a kwacha figure follows the rate. Saying so is
 ...
 *
 * Null until the cargo actually arrives in Lusaka. Cargo in the air has no
...
export type PublicShipment = {
  ...
  /**
   * How many of the customer's packages have arrived in Lusaka, and how many have
  ...
  /**
   * Today's published USD→ZMW rate, so the figures no invoice has frozen yet —
...
function buildTimeline(
  ...
  const stamps: Record<string, Date | null> = {
    ...
    RECEIVED_AT_ZAMBIA: shipment.arrivedAt,
   ...
   * from one sitting damaged on the shelf in Makeni. A stamp cannot lie about
   * that: the cargo either was checked in at Lusaka or it was not.
   */
  ...
  if (shipment) {
    ...
    if (invoice) {
      ...
      charge = {
        ...
        paid,
        outstanding,
        localCurrency: rate === null ? null : invoice.localCurrency ?? "ZMW",
        ...
        mayChange:
          outstanding <= 0
            ? null
            : `This amount can still change — storage is charged for every day after the first ${STORAGE_POLICY.freeDays}, and the kwacha figure follows the exchange rate on the day you pay. Collecting sooner keeps it lower.`,
      };
    }
    ...
    // Deliberately mirrors the release rule the Lusaka warehouse enforces, so the
    // page never tells a customer to come and collect cargo that will be
    // refused at the counter.
    const landed =
      shipment.status === "RECEIVED_AT_ZAMBIA" ||
    ...
    if (shipment.status === "DELIVERED") {
      ...
      baseNote =
        "Paid and ready. Bring your pickup note or this tracking number to our Makeni warehouse.";
    } else if (landed && charge && charge.outstanding > 0) {
      baseNote =
        "Your cargo has landed in Lusaka. Settle the balance and we will release it the same day.";
    } else if (landed) {
      baseNote =
        "Your cargo has landed and is being checked in. We will message you the moment it is ready.";
    } else if (shipment.status === "IN_TRANSIT") {
      baseNote =
        "In the air. We will message you the moment it lands in Lusaka.";
    } else {
      baseNote =
    ...
    const HOLD_MONEY_NOTE: Record<HoldStage, string> = {
      ...
      IN_TRANSIT:
        "Nothing is due from you while we trace it. No invoice is raised on cargo that has not been checked in at Lusaka.",
      IN_DAR:
    ...
    return {
      ...
      // cargo is in Lusaka or somewhere over the Indian Ocean. The stage says.
            ...
            return {
              ...
              label:
                collected === total && total > 0
                  ...
                  : `${arrived} of ${formatPackages(total, shipment.packageType)} arrived in Lusaka`,
            };
          })()
        : null,
      description:
