...
export type FollowUpRow = {
   ...
   * "手机配件" and the Lusaka desk reading this row gets "Mobile phone
...
/**
 * Everything a customer still owes us, ranked: the cargo standing in Lusaka with a
...
export async function followUpQueue({ credit = true }: { credit?: boolean } = {}) {
  ...
  */
  ...
  const shipments = await prisma.shipment.findMany({
    where: {
      status: { in: ["RECEIVED_AT_ZAMBIA", "READY_FOR_PICKUP"] },
    },
...
export function creditAttention(
  ...
  /* The exact dollar figure, only where the line above it is a conversion: with
     no rate published formatKwacha prints dollars itself, and the same amount
...
export async function supportOverview() {
  ...
  const [
    ...
    prisma.shipment.count({
      where: {
        status: { in: ["READY_TO_DEPART", "IN_TRANSIT", "RECEIVED_AT_ZAMBIA", "READY_FOR_PICKUP"] },
      },
    }),
...
/**
 * The same Zambian number, written the ways people actually write it.
 *
 * Numbers are stored as +260…, but a customer calling in reads theirs off the
 * handset as 0977… — and a clerk types what they hear. Matching only the stored
...
export function phoneVariants(query: string): string[] {
  ...
  if (digits.startsWith("0")) {
    ...
    // A bare 9xxxxxxxx — the part that is the same in both forms.
...
export async function searchShipments(query: string, take = 40) {
  ...
  return prisma.shipment.findMany({
    ...
    select: {
      ...
      // Unfinished cases only. A shipment can sit at RECEIVED_AT_ZAMBIA and still
