 ...
 * scanner this business has. The older payload was `ATC:P:<token>` — plain text
 ...
 * Tracking numbers run in sequence: AT-000042 tells you AT-000043 exists. This
 * same code is what the Lusaka counter scans to identify a box before releasing
...
export function parseQrPayload(raw: string): ScannedCode | null {
  ...
  const pkg = value.match(/^ATC:P:(.+)$/);
  if (pkg) return { kind: "package", token: pkg[1] };

  const shipment = value.match(/^ATC:S:(.+)$/);
  ...
  if (/^ATQ[\w-]{20,}$/.test(value)) return { kind: "shipment", token: value };
