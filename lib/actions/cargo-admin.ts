...
export async function deleteCargo(
  ...
  try {
    ...
    // and so resolves to exactly the owner and the admin. It read
    // shipment.purge until the admin role existed — purge is the owner's key
    ...
    // admin out of a REVERSIBLE soft delete and told them to ask management.
    if (cargo.status !== "READY_TO_DEPART" && !can(user.role, "shipment.cancel")) {
      return fail(
        t(
          locale,
          "This cargo has already left China. Only the owner or a admin can remove it now."
        )
      );
    }
