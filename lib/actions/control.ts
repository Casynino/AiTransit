...
/**
 * The admin's two controls: checking an account, and disputing a record.
 *
 * NEITHER OF THEM CHANGES ANYTHING THEY LOOK AT. That is the owner's rule and
 * it is the whole design: "do not silently change financial records". A admin
 * who disagrees with a payment does not edit the payment — Finance still sees
 * exactly what they entered, and a row beside it says a admin disputes it and
 ...
 * balance so the admin can see what they are comparing against, but that
...
const REVIEW_STATES = [
  ...
  /* Something is wrong with the record beyond the figures disagreeing, and a
     question asked of the desk that recorded it. Both are verdicts a admin
 ...
 * thing a admin or an auditor needs to read six months later: recorded, sent
 ...
 * RecordReview per record, still one audit line per record, because a bulk
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const targetId of ids) {
        ...
        const review = await tx.recordReview.create({
          data: {
            target: input.target,
            targetId,
            state: input.state,
