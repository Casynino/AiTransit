 ...
 * Splitting "record" from "pay" for every ZMW 20,000 would make the honest path
 * the slow one.
 */
...
const editSchema = z.object({
   ...
   * against the wrong account — petty cash when it went out of the bank. Absent
    ...
    const expenseNumber = await prisma.$transaction(async (tx) => {
      ...
      */
      ...
      /*
        ...
        has to be in the currency the money was actually in. A kwacha cost
      ...
      */
      ...
      const after = {
        ...
        ...(amountChanged
          ? {
              amount: new Prisma.Decimal(input.amount!),
              // The dollar value follows the kwacha at the rate this cost
