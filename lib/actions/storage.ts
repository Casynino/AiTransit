 ...
 * the cargo landed in Lusaka and whether it has been collected — so the only
 ...
 * the desk waived a million kwacha would look identical to a quiet one.
 *
 ...
 * the kwacha figure the customer was quoted, stored so it can never drift
 * from the bill, and storage was the one path that rewrote the total and left
 * it behind: the PDF and the invoice hero derived kwacha live from the new
 * total while the WhatsApp text and the public tracking page printed the stale
 * stored one, so a single message quoted two different kwacha amounts for the
