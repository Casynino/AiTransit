  ...
  try {
    ...
    if (!stillInChina && !(await canEditAfterDeparture(user))) {
      return fail(
        t(
          locale,
          "This cargo has already left China. Only the owner or a admin can correct it now."
        )
      );
    }
 ...
 * admin role arrived. shipment.cancel appears only in the ALL list, so it
 * resolves to the owner and the admin — the actual management predicate, and
