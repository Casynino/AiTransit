...
/** Lusaka warehouse confirms the batch has physically landed. */
      ...
      await recordAudit(
        {
          ...
          entity: "Batch",
          entityId: batchId,
          summary: `${batch.batchNumber} arrived in Lusaka`,
        },
        tx
      );
    });
...
/**
 * Does this kind of problem mean the cargo is standing in the Lusaka warehouse?
 *
 ...
 * Lusaka while the record said it was in the air: invisible to Warehouse
  ...
  if (!existing) {
    ...
    await tx.exceptionEvent.create({
      data: {
        ...
        action: "opened",
        note: `Raised at Lusaka check-in — ${EXCEPTION_TYPE_LABELS[core.type]}: ${core.description}`,
  ...
  if (changed) {
    ...
    await tx.exceptionEvent.create({
      data: {
        ...
        action: "note.added",
        note: `Re-checked at Lusaka arrival: ${core.description}`,
        actorId: core.raisedById,
      },
    });
  }
...
/**
 * Photographs taken on the Lusaka floor, filed against the cargo and — when there
  ...
  if (input.exceptionId) {
    await tx.exceptionEvent.create({
      data: {
        exceptionId: input.exceptionId,
        action: "photo.added",
        note: `${input.images.length} photo(s) taken on the Lusaka floor at check-in.`,
 ...
 * committed: packages received, shipment at RECEIVED_AT_ZAMBIA, history and audit
...
async function priceAfterCheckIn(
  ...
  try {
    ...
    console.error("Auto-pricing failed after a Lusaka check-in", {
      shipmentIds,
      error,
    });
    return false;
 ...
 *      RECEIVED_AT_ZAMBIA, arrivedAt stamped, packages ticked, a history line the
  ...
  try {
    const landed = await prisma.$transaction(async (tx) => {
      ...
      await attachArrivalPhotos(tx, {
        ...
        caption: outcome
          ? `${RECEIVING_OUTCOME_LABELS[outcome]} at Lusaka check-in`
          : "Lusaka check-in",
        ...
        await notifyReceivingOutcome(
          {
            ...
            raisedById: user.id,
            headline: `${shipment.trackingNumber} — ${RECEIVING_OUTCOME_LABELS[outcome].toLowerCase()} at Lusaka check-in`,
            detail:
              problem === "DAMAGED_CARGO" && severity
 ...
 * to RECEIVED_AT_ZAMBIA with a history line.
 *
      ...
      if (arriving.length > 0) {
        ...
        await tx.shipmentStatusHistory.createMany({
          data: arriving.map((shipment) => ({
            ...
            toStatus: "RECEIVED_AT_ZAMBIA" as const,
            location: "Lusaka warehouse",
            note: "Checked in with the rest of the manifest.",
            actorId: user.id,
          })),
 ...
 * the Lusaka floor pressing "Mark as arrived", and the storage clock starts from
