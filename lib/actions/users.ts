 ...
 * take the company.
 *
 * The owner has handed hiring to the admin: ADMIN holds `user.manage`, so a
 * admin opens accounts, assigns roles, suspends people and resets passwords
 ...
 * can hand one to himself. A admin able to mint an ADMIN, or to move a second
 ...
 *                      it, or a admin demotes the owner first and then does
 ...
 *                      direction — a admin does not reactivate a suspended
 ...
 * edit staff accounts", and the admin may — that is exactly what was handed
 ...
 * being demoted by his own admin while the guard reads "he is only Finance".
 *
...
/*
  ...
  reach the admin as toActionError's generic apology instead of the sentence
...
export async function createUser(
  ...
  try {
    ...
    */
    ...
    const warehouse =
      input.role === "CHINA_WAREHOUSE" || input.role === "ZAMBIA_WAREHOUSE";
    ...
    const user = await prisma.user.create({
      data: {
        ...
        // Guangzhou opens in Chinese; every Zambian desk opens in English.
  ...
  try {
    const outcome = await prisma.$transaction(async (tx): Promise<Guarded> => {
      ...
      /*
        ...
        and a admin reviving it is overruling that decision on the one account
      ...
      */
      ...
      const claimed = await tx.user.updateMany({
        ...
        // `status` is what a admin reads; `active` is what the sign-in check
  ...
  try {
    ...
    const outcome = await prisma.$transaction(async (tx): Promise<Guarded> => {
      ...
      /*
        ...
        screen looks different, and the admin simply signs in as the owner.
      */
      if (target.role === OWNER && actor.role !== OWNER) {
        await recordAudit(
          {
  ...
  try {
    const outcome = await prisma.$transaction(async (tx): Promise<Guarded> => {
      ...
      /*
        ...
        admin able to demote the owner to Finance, at which point there is no
        owner left to overrule anything and the admin runs the company by
        default.
