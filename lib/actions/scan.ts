 ...
 *
 * A clerk at the Lusaka counter points a phone at the sticker on a carton with the
...
export type ScanResult = {
   ...
   * The Lusaka counter is told the cargo is settled and which note settles it,
   * which is what it needs to hand a box over; the kwacha are Finance's
...
export async function resolveScan(
  ...
  if (!target) return fail(t(locale, "That is not a AITRANSIT label."));
...
async function describe(
    ...
    if (lock) {
      ...
      if (shipment.status === "RECEIVED_AT_ZAMBIA") {
        ...
        verdict = {
          ...
          detail: t(
            locale,
            "This cargo has not been checked in at the Lusaka warehouse yet."
          ),
        };
      }
  ...
  return ok({
    ...
    /*
      ...
      Guangzhou types "配件"; a Lusaka clerk holding the box sees "Accessories".
