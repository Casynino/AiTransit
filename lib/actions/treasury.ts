...
/*
  What a USD→ZMW rate can plausibly be.

  Wide enough to survive a decade of the kwacha, narrow enough to catch the
  mistake that actually happens: a kwacha figure typed into the dollar box, or
  ...
  try {
    const transferNumber = await prisma.$transaction(async (tx) => {
      ...
      /*
        ...
        A kwacha leg cannot be valued in dollars without one, and the form only
        asks for a rate when the two accounts differ in currency — so banking the
        day's mobile-money take, ZMW into ZMW and the commonest move this
        ...
        column and never reached the other's: one card printed K 0 against
        USD 2,000 and the other K 5,200,000 against USD 0.00, each card
      ...
      */
      ...
      if (needsRate && !valuationRate) {
        throw new Error(
          "No exchange rate is published, so a kwacha transfer cannot be valued in dollars. Publish one on Pricing & configuration first."
        );
      }
      ...
      /*
        ...
        kwacha side at today's, the dollar side at its face amount — and the
 ...
 * built: it counts only what this system has seen, and there is money in the bank
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      /*
        ...
        This used to record the kwacha anyway and value them at zero, on the
        grounds that the kwacha figure is the true one and must not be lost
        ...
        appears — it is a card that prints K 5,000,000 above USD 0.00 for good,
        and the only way back is a manual adjustment. A kwacha cost is already
      ...
      */
      ...
      if (input.amount > 0 && account.currency !== "USD" && !rate) {
        throw new Error(
          "No exchange rate is published, so a kwacha opening balance cannot be valued in dollars. Publish one on Pricing & configuration first."
        );
      }
      const amountUsd =
