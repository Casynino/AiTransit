 ...
 *
 * Anybody else's file needs `shipment.cancel` — the owner and the admin, and
 ...
 * off a consignment is not that act, and naming it that shut the admin out of
  ...
  try {
    ...
    if (!mine && !can(user.role, "shipment.cancel")) {
      return fail(
        t(
          locale,
          "Somebody else attached this file, so only the owner or a admin can take it off. Attach the right one instead — the list shows both."
        )
      );
    }
