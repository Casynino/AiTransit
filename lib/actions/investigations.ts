 ...
 *     writes onto the case — ExceptionEvent, which the Lusaka floor can read —
 ...
 *
 * Held by `exception.investigate` — the Lusaka floor, Support and the CEO. It is
...
export async function markCargoFound(
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      let target: ShipmentStatus = "RECEIVED_AT_ZAMBIA";
      ...
      await notify(
        {
          ...
          body:
            target === "READY_FOR_PICKUP"
              ? `Back in the Lusaka warehouse and cleared for pickup. ${input.note}`
              : `Back in the Lusaka warehouse. ${input.note}`,
 ...
 *
 * `exception.approve` is held by nobody but ADMIN — not by the Lusaka floor, which
 ...
 *
 * Not `Number(value)` and not zod's `.coerce.number()`: a kwacha figure that
...
const compensationSchema = z
  .object({
    ...
    currency: z
      ...
      .trim()
      .optional()
      .transform((v) => (v && v.length > 0 ? v.toUpperCase() : "ZMW"))
      .refine(
        (v): v is "ZMW" | "USD" => v === "ZMW" || v === "USD",
        "Currency must be ZMW or USD."
      ),
    paidAt: z
      .string()
      .trim()
 ...
 *
 * Held by `exception.compensate` — Finance and the CEO. The Lusaka floor cannot
    ...
    if (input.currency === "USD") {
      ...
      if (!published) {
        return fail(
          t(
            locale,
            "No exchange rate is published, so a kwacha cost cannot be valued in dollars. Publish one on Pricing & configuration first."
          )
        );
      }
      rate = published;
    ...
    const shipmentId = await prisma.$transaction(async (tx) => {
      ...
      */
      ...
      await logEvent(tx, {
        ...
        // Deliberately no amount and no currency. The Lusaka floor reads this
