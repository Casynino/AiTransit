...
/*
  ...
  the admin, the admin approves it, and only THEN is the money deducted as
...
/** The screens a run appears on: Finance builds it, the admin decides it. */
const PAYROLL_PATHS = [
  "/app/finance/payroll",
  "/app/admin/payroll",
  "/app/admin/approvals",
...
const NOT_EDITABLE: Record<string, string> = {
  PENDING_APPROVAL:
    "is with the admin and is frozen while it waits. It has to come back before a figure on it can change.",
 ...
 * the step: the admin has to be agreeing to a payment out of a named account
  ...
  try {
    const code = await prisma.$transaction(async (tx) => {
      ...
      /* The claim, and what it stops is two clicks on one button: the second
         ...
         second submission time over the age the admin is being shown. */
...
/**
 * The admin agrees the month, or sends it back with a reason.
 *
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      /*
        ...
        The permissions alone do not stop this: the owner and the admin both
 ...
 * that now settles immediately, and the admin's one-action run. Whatever the
    ...
    locale: Awaited<ReturnType<typeof viewerLocale>>;
      ...
      if (account.currency !== "USD" && !rate) {
        throw new Error(
          t(
            locale,
            "No exchange rate is published, so a dollar salary bill cannot be paid out of a kwacha account. Publish one on Pricing & configuration first."
          )
        );
      }
      ...
      /*
        ...
        Two admins pressing pay at the same moment both read APPROVED; the
 ...
 * expense.
 *
 * The run is denominated in dollars and the account may be in kwacha, so the
...
/**
 * THE ADMIN RUNS THE MONTH HIMSELF, in one action.
 *
 * The owner: "admin can just run a payroll and the money will be deducted".
 * The two-step month — Finance builds and sends up, the admin accepts — keeps
