  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      /*
        ...
        The price is worked out from the weight and the piece count the Lusaka
      ...
      */
      if (
        ...
        throw new Error(
          `${shipment.trackingNumber} ${t(locale, "has not been checked in at Lusaka yet, so there is no final weight to price. The system raises the bill by itself the moment the warehouse checks it off the manifest.")}`
        );
      }
 ...
 *
 *  1. Once a single kwacha has landed, the invoice is frozen. A bill that can
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      */
      ...
      // and sends the bill, but moving the USD→ZMW rate on one invoice changes
...
> {
  ...
  try {
    const receiptNumber = await prisma.$transaction(async (tx) => {
      ...
      //
      // A USD invoice settled in kwacha converts at the rate frozen onto
      ...
      // this money: kwacha do not go into a dollar account, and an account
      ...
      // over, which may be kwacha; the bill is settled by `credited`.
...
export async function issuePickupNote(
  ...
  try {
    const noteNumber = await prisma.$transaction(async (tx) => {
      ...
      if (shipment.status !== "RECEIVED_AT_ZAMBIA") {
        throw new Error(
          "Cargo must be checked in at the Lusaka warehouse before a pickup note can be issued."
        );
      }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (note.shipment.status === "READY_FOR_PICKUP") {
        await tx.shipment.update({
          ...
          data: { status: "RECEIVED_AT_ZAMBIA", readyForPickup: null },
        });
        await tx.shipmentStatusHistory.create({
          data: {
...
export type BillableHit = {
  ...
  paid: number;
  outstanding: number;
  /** The rate frozen on the bill, so a kwacha payment converts at what was quoted. */
  rate: number | null;
  status: string;
};
...
export async function searchBillable(query: string): Promise<BillableHit[]> {
  ...
  // Lusaka desk has to recognise the right consignment before taking money for it.
  const locale = await viewerLocale();
 ...
 * arrive in Lusaka and could never be billed again. What is kept instead is the
