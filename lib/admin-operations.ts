 ...
 * composition, the Lusaka floor's snapshot, the receiving queue, the approvals
 ...
 *
 * That restraint is the whole point of the file. A admin's operations screen
 ...
 * NARROWER population than this file does: its "on the floor" is RECEIVED_AT_ZAMBIA
 ...
 *   Lusaka, which the same screen then counts again as the check-in backlog. The
 ...
 *
 *   zambiaStats() gives four cargo counts and two batch counts. Only awaitingCheck
...
export type OpsDesk = {
  ...
  key: "china" | "zambia" | "finance" | "support";
  desk: string;
  rows: OpsDeskRow[];
};
...
export async function adminOperations(
  ...
  const chinaLeg: OpsLeg = {
    ...
    /*
      ...
      two disagree by one, and a admin who spots that difference is right to
  ...
  */
    ...
    stages: [
      {
        ...
        /* Counted in pieces, like every other row in this column. The FLIGHTS
           those pieces are riding are the Lusaka card's queue below — the two
      ...
      },
      {
        ...
        /* floorComposition().held, which is cargo with no live pickup note AND
           ...
           has to say which of the two it is, or the admin reading the Lusaka
        ...
        href: "/app/cargo?status=RECEIVED_AT_ZAMBIA",
        permission: "shipment.view",
        tone: "plain",
      },
      {
      ...
      {
        key: "flagged",
        /* The third slice of the floor, and the Lusaka card below does not repeat
  ...
  /* Cargo already on the Lusaka floor whose days are the finding. agingInWarehouse
  ...
  /* Finance's own queues, and only those. `claims` in the approvals engine is a
     cargo case — it is answered on the Lusaka column above, and counting it as
      ...
      rows: [
        {
          ...
          /* "every desk", because todaySummary() counts every shipment photo
             filed since midnight — Lusaka's damage shots and delivery proofs among
      ...
      rows: [
        ...
        },
        {
          ...
          /* NOT "landed". financeStats().awaitingInvoice counts cargo at
             RECEIVED_AT_ZAMBIA *and* IN_TRANSIT whose only invoice is a draft or
             ...
             the admin and the finance clerk two answers to one question. */
  ...
  return {
    ...
    /* All three off the receiving queue's in-air batch rows, so the bar's
       ...
       the airborne headline AND into the Lusaka check-in backlog on the same
       screen — cargo counted twice, in two places a admin acts on. */
    corridor: {
      cargo: inAirCargo,
