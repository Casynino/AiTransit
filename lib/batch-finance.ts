...
export type BatchFinance = {
  ...
  /**
   * The same figure in kwacha. Invoiced cargo contributes the rate frozen
  ...
  expenseCount: number;
  /**
   * The same costs in kwacha, summed the accurate way.
   *
   * NOT expensesUsd × today's rate. A cost paid in kwacha is already an
   * exact kwacha figure; converting it to dollars and back rounds it twice
   * and lands a few kwacha away from what actually left the account. Summed
...
export async function batchFinance(
  ...
  /*
    ...
    they were recorded, so a kwacha cost and a dollar cost can be added
