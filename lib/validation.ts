...
export const paymentSchema = z.object({
   ...
   * in kwacha at the counter, and recording it as USD would put a figure on
   ...
   */
  currency: z.enum(["USD", "ZMW"]).optional(),
  /**
  ...
  /**
   * Which company account the money landed in — the bank account, the mobile-money
  ...
  paidAt: z
    ...
    .refine(
      // A whole day of slack, so a clerk in Lusaka is never told their own
...
export const userSchema = z.object({
  ...
  /*
    ...
    This was a hand-typed list of five, and when the Admin role was added the
    dropdown offered it while this refused it — so creating a admin failed with
    a raw validator message on the owner's screen. Changing an existing person TO
    admin worked the whole time, because that path checks against ROLE_LABELS,
  ...
  */
  ...
  employeeId: z
    .string()
    .trim()
    .max(24, "Employee IDs are short — AT-014, not a sentence.")
    .optional()
