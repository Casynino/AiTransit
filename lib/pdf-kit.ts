 ...
 * is how "Guangzhou → Lusaka" once reached a customer as garbage.
 */
...
export function createSheet(options: SheetOptions) {
  ...
  function furniture(n: number) {
    ...
    put("SHIP WITH YOUR OWN, PROUDLY ZAMBIAN", MARGIN, FOOT_TOP + 39, {
      size: 6.8,
      style: "bold",
      colour: RED,
    });
  ...
  function table(input: {
    ...
    /** Extra note printed under the table, e.g. the kwacha conversion. */
    note?: string;
  }) {
