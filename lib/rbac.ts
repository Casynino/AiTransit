...
export type Permission =
   ...
   *
   * Not folded into `shipment.edit`, and the reason is the Lusaka floor. Editing is
   * gated on the cargo still sitting in China precisely so Lusaka cannot rewrite
   * China's weights and counts — but Lusaka is the desk holding the customs entry
   * and the signed damage report. Reusing shipment.edit here would have forced a
   * choice between "Lusaka cannot file the paperwork it is holding" and "Lusaka can
  ...
  /// after that reads it — Lusaka scans it, Finance quotes the number off it — and
  /// none of them has any reason to make another one. A second sticker for the
  /// same cargo, printed in Lusaka because a label was scuffed, is two identities
  ...
  | "batch.receive" // mark arrived in Lusaka
  ...
  /// this, because a case is never one department's business: Lusaka found the
  ...
  | "pickupNote.issue"
  | "pickupNote.cancel"
  | "fx.manage" // publish the USD→ZMW rate
  /**
  ...
  // Delivery
  | "shipment.release"
  | "delivery.history" // cargo already handed over — the Lusaka release log
  // Lusaka warehouse floor
  | "inventory.view" // cargo physically held at the Lusaka warehouse
...
const CHINA: Permission[] = [
  // This floor's own throughput, not Lusaka's. The report branches on the reader's
  ...
  "customer.view",
  "customer.manage",
  // Reads the Issues & Claims, and nothing more. When Lusaka reports a box
...
/**
 * Lusaka warehouse — the desk that receives cargo in Zambia and hands it over.
 *
 ...
 * them to Lusaka does not give Lusaka a way to correct cargo it is holding; it gives
 * Lusaka a way to rewrite and soft-delete China's cargo that has not flown yet —
 ...
 * confirms payment and issues the pickup note, Lusaka only scans it.
 */
const ZAMBIA: Permission[] = [
  "shipment.view",
  /* Lusaka too: they open the box on arrival, and a mislabelled consignment is
  ...
  "exception.raise",
  "exception.resolve",
  // Lusaka works the investigation: reports missing, uploads damage evidence,
  ...
  // found the box or agreed the damage. The CANNOT list bars Lusaka from closing
...
const FINANCE: Permission[] = [
  ...
  "pickupNote.issue",
  "pickupNote.cancel",
  // Finance quotes kwacha all day, so Finance keeps the rate current. The
  ...
  */
  ...
  /*
    ...
    salary payroll for everyone but the admin is the one who pays". The grant
...
const ALL: Permission[] = Array.from(
  new Set<Permission>([
    ...
    */
    ...
    // Held by nobody else — including the Lusaka floor, which the owner's CANNOT
    ...
    // by the warehouse at all; Lusaka drives one to CARGO_FOUND or hands it over,
    // and the CEO signs it off. If the owner later wants Lusaka to close its own
    // no-money cases, that is one line: add "exception.close" to ZAMBIA and gate
    ...
    /* Checking an account against something outside this system, and disputing
       ...
       resolve to the owner and the admin — the two chairs that answer for the
...
/**
 * The customer, signing in to their own portal.
 *
 * Deliberately EMPTY, and that is the whole design. Every staff page and every
 * server action asks for a permission before it does anything, so a role that
 * holds none of them is refused by machinery that already exists rather than by
 * a new set of "is this a customer?" checks scattered through the app — which is
 * the version that eventually misses one.
 *
 * What a customer may see is therefore not expressed here at all. The portal
 * under /portal never calls `authorize`; it resolves the signed-in user to the
 * Customer record linked to their account and filters every single query by that
 * id. The question "may they see this?" is answered by "is it theirs?", which is
 * the only question that has a safe answer for a customer.
 */
const CUSTOMER: Permission[] = [];

/**
 * AITRANSIT has FIVE staff roles and no sixth chair.
 *
 * Target Express split oversight in two: an owner who could change the system
 * and a manager who could run the business but not rewrite the rules. AITRANSIT
 * folds those into ADMIN, at the specification's instruction — so ADMIN is ALL,
 * literally the whole permission list, and there is no subtracted second role
 * below it. Approving credit, reviewing transactions, reconciling accounts,
 * reading batch profit and loss, managing users, prices, accounts and settings,
 * and reading the activity log all resolve to this one role.
 */
export const ROLE_PERMISSIONS: Record<Role, Permission[]> = {
  ADMIN: ALL,
  CHINA_WAREHOUSE: CHINA,
  ZAMBIA_WAREHOUSE: ZAMBIA,
  FINANCE,
  CUSTOMER_CARE,
  CUSTOMER,
};
...
export const ROUTE_PERMISSIONS: { prefix: string; permission: Permission }[] = [
  ...
  // The Lusaka warehouse floor. The arrivals board and the verification bench
  ...
  /* The money desk. `fx.manage` for both — reading the queue and quoting a
     request are the same desk's job. Marking one COMPLETE needs the separate
     authority to say money moved, and that is checked on the request itself
     rather than at the door, because the same person reads both. */
  { prefix: "/app/finance/exchange", permission: "fx.manage" },
  { prefix: "/app/finance/fx-board", permission: "fx.manage" },
  /* The supplier-payment register is a set of company figures, so it opens on
     accounting.view like the rest of the books. Recording a new one is gated
     separately on payment.record inside the page. */
  { prefix: "/app/finance/supplier-payments", permission: "accounting.view" },
  ...
  /* The oversight pages the withdrawn management role used to hold, now part
     of /app/admin. Each keeps the permission it was guarded on before the move
     rather than inheriting the bare /app/admin rule below — Finance holds
     report.view, and the one desk these pages exist to check must not be able
     to stand inside the workspace that checks it. Longest prefix wins, so
     these resolve ahead of the catch-all. Every action inside still re-checks
     its own permission at the server; a route guard decides which door opens,
     never what may be done once through it. */
  { prefix: "/app/admin/reconciliation", permission: "account.reconcile" },
  { prefix: "/app/admin/payroll", permission: "payroll.approve" },
  { prefix: "/app/admin/batches", permission: "profit.view" },
  { prefix: "/app/admin/approvals", permission: "record.review" },
  { prefix: "/app/admin/control", permission: "record.review" },
  { prefix: "/app/admin/operations", permission: "record.review" },
  { prefix: "/app/admin/finance", permission: "record.review" },
  { prefix: "/app/admin/reports", permission: "record.review" },
  /* The control centre at the bare /app/admin, and everything under it that is
     not named above. record.review rather than report.view for the same reason:
     this is the reviewer's workspace, not a report Finance may read. */
  { prefix: "/app/admin", permission: "record.review" },
