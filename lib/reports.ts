 ...
 * kwacha, and a column that silently mixes the two is worse than no column.
 * Where the original currency matters — a kwacha cost, a kwacha account —
...
export type ReportColumn = {
  ...
  numeric?: boolean;
  /**
   * A USD figure that may be restated in kwacha.
   *
...
export type ReportUnit = "USD" | "ZMW";
 ...
 * USD report, if I'm on K I should get a K report — not two." Printing
...
export function presentReport(
  ...
  const toKwacha = opts.unit === "ZMW" && opts.rate !== null;
  const unit: ReportUnit = toKwacha ? "ZMW" : "USD";
  const unitLabel = toKwacha ? "K" : "USD";
  ...
  const convert = (value: string | number | null) => {
    if (!toKwacha || typeof value !== "number") return value;
  ...
  const columns = report.columns.map((c) =>
    c.money
      ? {
          ...c,
          /* "USD" as a whole label becomes "K"; anything else gains the unit,
  ...
  return {
    ...
    currencyNote: (() => {
      /*
        ...
        kwacha entries printed "Money shown in dollars, as billed." under it
      ...
      */
      ...
      if (toKwacha) {
        return `Money shown in kwacha, converted at USD 1 = K ${(opts.rate as number).toLocaleString("en-US")}, the rate in force when this was printed.${
          native.length > 0
            ? ` ${native.map((c) => c.label).join(", ")} are each in the account's own currency, shown in the Currency column, and are not converted.`
            : ""
        }`;
      }
      return opts.unit === "ZMW"
...
export const REPORTS = [
  ...
  /* The two the owner asked for that the register could not answer.
     ...
     They are here rather than on a admin-only screen because the register IS
     the report system — a payroll report that only the admin's page knew about
  ...
  /* AITRANSIT's second line of business. Two reports because they answer two
     different questions: what the money DESK handled, and what actually left
     the business for a supplier. A request that was quoted and never taken up
     belongs in the first and must never appear in the second. */
  { key: "exchange-requests", label: "Money exchange requests" },
 ...
 * two million kwacha must not read like a month where nothing happened.
 */
 ...
 * a month where the counter forgave two million kwacha looked exactly like
...
async function storage(f: ReportFilters): Promise<ReportResult> {
  ...
  return {
    ...
    caption:
      `Every consignment that landed in Lusaka in this period, how long it stayed and what that cost. ${STORAGE_POLICY.freeDays} days are free from the day it arrives; USD ${STORAGE_POLICY.perDayUsd} a day after that, and the clock stops when the cargo is collected. Calculated is what the policy produced, Charged is what reached a bill, and Waived is what the desk chose not to collect — read that column, it is the only record of what goodwill and our own delays cost. ${held} consignment(s) are charging right now; ${forgiven} had the fee waived.`,
    columns: [
      { key: "tracking", label: "Tracking" },
      { key: "customer", label: "Customer" },
      { key: "batch", label: "Batch" },
      { key: "arrived", label: "Arrived Lusaka" },
...
function ledgerReport(
  ...
  /*
    ...
    currency, and the company holds both kwacha and dollar accounts. Running
    them into a single figure produced a balance that added K 1,200,000 to
...
const PAYROLL_STATE: Record<string, string> = {
  DRAFT: "Being built",
  PENDING_APPROVAL: "With the admin",
...
async function financialStatement(f: ReportFilters): Promise<ReportResult> {
  ...
  return {
    ...
    caption: rate
      ...
      */
      ? "The whole position on one page. Kwacha balances are valued at the published rate; the full statement is the document above."
      : "The whole position on one page. No exchange rate is published, so kwacha balances are shown at their recorded dollar value.",
    columns: [
      { key: "line", label: "Line" },
      { key: "usd", label: "USD", numeric: true, money: true },
    ],
...
/**
 * What the money desk handled in the window.
 *
 * Counts REQUESTS, not money. A booking that was quoted and never taken up is a
 * real piece of work and belongs in this report; it moved nothing, so it must
 * not appear in any figure the business's cash position is built from. The
 * money column is therefore the amount ASKED FOR, in the currency it was asked
 * in, and it is deliberately not totalled — summing kwacha, dollars and RMB
 * into one number would produce a figure in no unit at all.
 */
async function exchangeRequests(f: ReportFilters): Promise<ReportResult> {
  const rows = await prisma.exchangeRequest.findMany({
    where: { ...(range(f) ? { createdAt: range(f) } : {}) },
  ...
  return {
    ...
    title: "Money exchange requests",
    caption:
      "Every currency booking, quotation and China payment request in the window, whatever came of it. The amount is what the customer asked for, in the currency they asked in — it is not totalled, because adding kwacha to dollars to RMB gives a number in no unit. Only rows marked Completed moved any money; those are in Supplier payments and in the ledger.",
    columns: [
      ...
      { key: "customer", label: "Customer" },
      { key: "pair", label: "Pair" },
      { key: "amount", label: "Amount asked", numeric: true },
      { key: "rate", label: "Agreed rate", numeric: true },
      { key: "recipient", label: "Recipient" },
      { key: "status", label: "Status" },
      { key: "reviewedBy", label: "Handled by" },
      { key: "account", label: "Account" },
    ],
    rows: rows.map((row) => ({
      reference: row.reference,
      date: row.createdAt.toISOString().slice(0, 10),
      type: row.type.replace(/_/g, " ").toLowerCase(),
      customer: row.customer?.name ?? row.contactName,
      pair: `${row.fromCurrency} → ${row.toCurrency}`,
      amount: money(toNumber(row.amount)),
      rate: row.agreedRate === null ? "—" : toNumber(row.agreedRate),
      recipient: row.recipientName ?? "—",
      status: row.status.replace(/_/g, " ").toLowerCase(),
      reviewedBy: row.reviewedBy?.name ?? "—",
...
/**
 * Money actually paid to suppliers in China.
 *
 * Totalled in USD only, because USD is the one column every row can be
 * expressed in — and rows recorded without an exchange rate have no USD figure
 * and are therefore MISSING from the total. The caption says so rather than
 * quietly under-reporting, which is the whole reason the column is nullable.
 */
async function supplierPayments(f: ReportFilters): Promise<ReportResult> {
  const rows = await prisma.supplierPayment.findMany({
    where: {
      status: { not: "CANCELLED" },
      ...(range(f) ? { createdAt: range(f) } : {}),
      ...(f.accountId ? { accountId: f.accountId } : {}),
    },
    orderBy: { createdAt: "desc" },
    take: 2000,
  ...
  const unvalued = rows.filter((row) => row.amountUsd === null).length;
  ...
  return {
    ...
    caption:
      unvalued > 0
        ? `Money paid to suppliers in China for customers. The USD total covers only the rows that carry an exchange rate — ${unvalued} row(s) here do not, and are excluded from it rather than converted at a guess.`
        : "Money paid to suppliers in China for customers, with the account each payment left and the customer it was for.",
    columns: [
      { key: "reference", label: "Reference" },
      { key: "date", label: "Recorded" },
      { key: "customer", label: "Customer" },
      { key: "supplier", label: "Supplier" },
      { key: "order", label: "Their order" },
      { key: "cargo", label: "Cargo" },
      { key: "amount", label: "Sent", numeric: true },
      { key: "currency", label: "Currency" },
      { key: "usd", label: "In USD", numeric: true, money: true },
      { key: "fee", label: "Our fee", numeric: true, money: true },
      { key: "account", label: "Paid from" },
      { key: "status", label: "Status" },
      { key: "handledBy", label: "Recorded by" },
    ],
    rows: rows.map((row) => ({
      reference: row.reference,
      ...
      order: row.supplierReference ?? "—",
      cargo: row.shipment?.trackingNumber ?? "—",
      amount: money(toNumber(row.amount)),
      currency: row.currency,
      usd: row.amountUsd === null ? "—" : money(toNumber(row.amountUsd)),
      fee: row.serviceFeeUsd === null ? "—" : money(toNumber(row.serviceFeeUsd)),
      account: row.account?.name ?? "—",
      status: row.status.toLowerCase(),
      handledBy: row.handledBy?.name ?? "—",
    })),
    totals: {
      usd: money(
        rows.reduce(
          (sum, row) => sum + (row.amountUsd === null ? 0 : toNumber(row.amountUsd)),
          0
        )
      ),
      fee: money(
        rows.reduce(
          (sum, row) =>
            sum + (row.serviceFeeUsd === null ? 0 : toNumber(row.serviceFeeUsd)),
          0
        )
      ),
    },
...
export async function runReport(
  ...
  switch (key) {
    case "exchange-requests":
      return exchangeRequests(filters);
    case "supplier-payments":
      return supplierPayments(filters);
    case "income":
    ...
    case "mobile-money":
      return ledgerReport(
        "mobile-money",
        "Mobile money",
        "Every movement through Airtel Money, MTN MoMo and any other mobile account.",
