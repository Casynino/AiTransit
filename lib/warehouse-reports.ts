...
/**
 * The Lusaka warehouse's own reporting.
 *
...
export async function warehouseReport(periodDays: number, locale: Locale = "en") {
  ...
  const [
    ...
    // ticked off the manifest. The only leg on this page Lusaka fully controls.
    prisma.shipment.findMany({
    ...
    prisma.user.findMany({
      where: { role: "ZAMBIA_WAREHOUSE", active: true, status: "ACTIVE" },
  ...
  // Everyone currently on the Lusaka roster appears even at zero — a name with no
  // numbers is itself worth seeing — plus anyone else who touched the floor in
  // the window (a admin covering a shift, the CEO releasing on a Sunday).
  for (const staff of zambiaStaff) {
    if (!tally.has(staff.id)) tally.set(staff.id, blank(staff.id));
  }

  const names = new Map(zambiaStaff.map((s) => [s.id, s.name]));
  ...
  const awaitingPayment =
    heldNow.find((row) => row.status === "RECEIVED_AT_ZAMBIA")?._count._all ?? 0;
