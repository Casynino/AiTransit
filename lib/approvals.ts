...
export type QueueDef = {
  key: QueueKey;
  label: string;
  /** What deciding it means, in the admin's words, not the schema's. */
  detail: string;
  href: string;
...
const QUEUES: Record<QueueKey, QueueDef> = {
  ...
  payments: {
    ...
    detail: "Money a desk says arrived, not yet banked. Kwacha at today's rate.",
  ...
  drafts: {
    ...
    /* Not /app/finance/invoices?status=DRAFT. That route is a bare redirect to
       the finance overview and the status is dropped on the way, so a admin
  ...
  statements: {
    ...
    /* The closed-batches sheet, not the batch page and not the admin's own
...
export type ApprovalQueue = QueueDef & {
   ...
   * to add kwacha to dollars. A queue that cannot state its value states
...
export async function approvalQueues(now = new Date()): Promise<ApprovalQueue[]> {
  ...
  /* Submissions are keyed in the currency the customer paid in — ZMW or USD,
     the only two the claim form offers (lib/actions/collections.ts) — and
     kwacha are what most of them arrive in. Summing the dollar ones alone
     left this row understating the money waiting to be verified, in exactly the
     currency it mostly waits in, on a page read in kwacha.
     ...
     Null rather than a partial sum when a kwacha claim is waiting and no rate
  ...
  // Rounded once, at the end: dividing kwacha by a rate leaves fractions of
