...
export type NavItem = {
  ...
  /** Match child routes too (e.g. /app/cargo/AT-000123). */
  exact?: boolean;
  /**
...
/**
 * Lusaka warehouse gets its own menu rather than a filtered slice of the shared
 * one.
 *
 ...
 */
/**
 * The Lusaka floor's menu, grouped the way the desk is organised.
 *
 ...
 */
const ZAMBIA_SECTIONS: NavSection[] = [
  {
    title: "Top",
    items: [
      {
...
const FINANCE_SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        /* This desk BUILDS the run and cannot pay it; the admin agrees it
 ...
 *
 * Same skeleton as Lusaka, Finance and Support: Home alone, the work in the kinds
 ...
 * this floor rather than Lusaka's.
 */
...
const ADMIN_SECTIONS: NavSection[] = [
  {
    ...
    items: [
      { href: "/app/dashboard", label: "Home", icon: "LayoutDashboard", exact: true },
      /* The command centre the second management chair used to open on. AITRANSIT
         has no second chair, so it opens here — beside Home rather than behind a
         role, because the person reading it is the same person either way. */
      { href: "/app/admin", label: "Control centre", icon: "Gauge", exact: true, permission: "record.review" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      /* The money desk. AITRANSIT exchanges currency and pays suppliers in
         China, and both are worked from the same inbox — so they sit together
         and beside the ledger they eventually feed. */
      { href: "/app/finance/exchange", label: "Money desk", icon: "Coins", permission: "fx.manage" },
      { href: "/app/finance/supplier-payments", label: "Supplier payments", icon: "HandCoins", permission: "accounting.view" },
      { href: "/app/finance/fx-board", label: "Exchange board", icon: "ArrowLeftRight", permission: "fx.manage" },
  ...
  {
    /* Everything the withdrawn management role used to hold. Grouped rather than
       scattered because these are the checks — read at the end of a day or a
       month — and not the work that fills one. */
    title: "Oversight",
    group: { label: "Oversight", icon: "ShieldCheck" },
    items: [
      { href: "/app/admin/operations", label: "Operations board", icon: "Boxes", permission: "record.review" },
      { href: "/app/admin/finance", label: "Finance oversight", icon: "TrendingUp", permission: "record.review" },
      { href: "/app/admin/batches", label: "Batch finances", icon: "Plane", permission: "profit.view" },
      { href: "/app/admin/reconciliation", label: "Reconciliation", icon: "Scale", permission: "account.reconcile" },
      { href: "/app/admin/approvals", label: "Pending approvals", icon: "BadgeCheck", permission: "record.review" },
      { href: "/app/admin/control", label: "Control room", icon: "ShieldCheck", permission: "record.review" },
      { href: "/app/admin/payroll", label: "Payroll approval", icon: "Wallet", permission: "payroll.approve" },
      { href: "/app/admin/reports", label: "Management report", icon: "FileText", permission: "record.review" },
    ],
  },
  {
...
const ROLE_SECTIONS: Partial<Record<Role, NavSection[]>> = {
  ...
  ZAMBIA_WAREHOUSE: ZAMBIA_SECTIONS,
