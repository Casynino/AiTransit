 ...
 *    dashboard read, so a admin quoting this page and a clerk quoting that
 ...
 *    point — the admin's screen must be able to DISAGREE with nobody, and a
...
export type CheckSide = {
   ...
   *
   * The screen leads in kwacha everywhere else in this app and it cannot do
...
export type AccountPosition = {
   ...
   *  at, never today's. Small print beside a kwacha balance, never a total. */
  balanceUsd: number;
  entries: number;
...
export type PositionGroup = {
   ...
   *
   * The bank group holds kwacha and dollars. Adding them needs a rate, and
...
export async function reconciliation(locale: Locale = "en") {
  /* The month, and the month before it — the same window the P&L opens on, so
     a admin reading this page and the profit page on the same morning is
  ...
  /* ---------------------------------------- 4. accounts below zero
     ...
     a setup step nobody did, not a booking error, and telling a admin to go
  ...
  */
  ...
  /* ---------------------------------------- 5. what each account holds
     ...
     Not a sixth check — a statement of position, which is the thing a admin
  ...
  */
  ...
  /* ---------------------------------------- 6. collections
     ...
     by status — and a admin signing anything off wants to know the two
  ...
  const collectionChecks: Check[] = [
    ...
    },
    {
      ...
      /* Deliberately a count and not an amount, and said so. A submission is
         ...
         would add kwacha to dollars — see lib/collections.ts, which refuses
         the same total for the same reason. */
      expected:
        "No money figure is shown for these on purpose: a claim is stored in the currency the customer sent, so adding them together would add kwacha to dollars. The count is the honest figure until Finance rules on each one.",
