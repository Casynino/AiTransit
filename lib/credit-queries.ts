...
export type CreditRow = {
  ...
  /** The frozen rate on this bill, so the kwacha figure never moves. */
 ...
 * forgiven two million kwacha for must not read like a customer who has
  ...
  return {
    lastPayment: payment
      ? {
          ...
          /* The debt's currency, not the till's: a customer hands over kwacha
  ...
  return customers.map((customer) => {
    ...
    return {
      ...
      /* Forgiven credit, carried beside the position rather than dropped from it.
         ...
         column a customer the desk has written two million kwacha off for
