...
/**
 * How a kwacha amount is written on screen.
 *
 * ZMW is the ISO code and stays in the database, on invoices and anywhere a
 * bank or a customs form will read it. On screen the business writes K, so
 ...
 */
const DISPLAY_CURRENCY: Record<string, string> = { ZMW: "K" };

export function formatMoney(value: Numeric, currency = "ZMW") {
...
/**
 * Zambian numbers get typed in half a dozen shapes (0965…, +260965…, 260965…).
 * Normalise to +260XXXXXXXXX so customer lookup actually matches.
 *
 * The country code is 260. It was 255 — Tanzania — throughout the Target
 * Express original, and a stale one here is worse than a wrong label: every
 * customer created at the Guangzhou counter would be stored under a Tanzanian
 * number, and Customer.phone is UNIQUE, so two genuinely different Zambian
 * numbers could never collide but a lookup by the number a customer reads off
 * their own handset would find nothing.
 */
...
export function normaliseCode(input: string) {
   ...
   *
   * Tracking numbers are AT-000131, and people type AT000131, at131, or read
   ...
   * exist" — and now that the label carries a typeable /track/AT-000131 link,
   * a missed dash is a customer who thinks we lost their box.
   *
   * Only AT followed by digits is touched. Batch numbers (BATCH-2026-001) and
   * China batch codes (GZ-SHIP-2026-001) carry letters and more than one dash,
   * and are left exactly as typed.
   */
  const tx = value.replace(/-/g, "").match(/^AT(\d{1,6})$/);
  if (tx) return `AT-${tx[1].padStart(6, "0")}`;
