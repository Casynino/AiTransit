...
import { zambiaStats, executiveStats } from "@/lib/queries";
import {
  statementSummary,
  submittedStatements,
 ...
 * of them already aggregates, because the admin's screen and the desk's own
 ...
 * The screen must say so rather than let a admin read it as attendance.
 *
...
export type AdminOverview = {
  /** USD→ZMW as published. Null when no rate exists — then figures stay in USD. */
  rate: number | null;
  ...
  operations: {
    ...
    /** Consignments physically held in Lusaka, cleared or not. */
  ...
  finance: {
    ...
    /*
      ALREADY IN KWACHA — do NOT pass these through formatKwacha.
      ...
      account's own currency is known — a kwacha account is added as it stands
  ...
  staff: {
    ...
    payroll: {
      ...
      /*
        ...
        printed the newest run's total beside it, so the admin was shown
...
export type InsightSources = {
  ...
  queues: ApprovalQueue[];
  statements: StatementSummary;
  payroll: AdminOverview["staff"]["payroll"];
};
...
export async function adminOverview(
  ...
  const [
    ...
    /* Batches still in Guangzhou, on the named set above. Nothing else counts
       ...
       figure that already includes flights in the air, and zambiaStats answers
  ...
  const payroll: AdminOverview["staff"]["payroll"] = latest
    ? {
  ...
  /*
    KWACHA ACCOUNTS ARE ADDED AS THEY STAND, and only a dollar account is
    converted — once.

    Summing `balanceUsd` and letting the screen multiply by today's rate double-
    converts every kwacha account: the ledger's dollar column was struck at
  ...
  */
  ...
  const overview: Omit<AdminOverview, "insights"> = {
    rate,
    ...
    operations: {
      ...
      /* The floor's own count, which includes cleared cargo still standing
         here. zambiaStats.inWarehouse answers a narrower question (not yet
