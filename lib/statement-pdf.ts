 ...
 *
 * ONE CURRENCY. It used to print kwacha and dollars side by side, and the
 * owner's instruction was to stop: "if I'm on USD I should get a USD report,
 * if I'm on K I should get a K report — not two." The switch on the profit
...
export function statementToPdf(input: {
  ...
  pl: Pl;
  prior: Pl;
  /** USD → ZMW in force when this was printed. Null when none is published. */
  rate: number | null;
  ...
  const inKwacha = input.unit === "ZMW" && rate !== null;
  const unitLabel = inKwacha ? "K" : "USD";

  /** Every figure on the page, in the one currency this document is written in. */
  const fmt = (usd: number) =>
    inKwacha
      ? `K ${Math.round(usd * (rate as number)).toLocaleString("en-US")}`
  ...
  const sheet = createSheet({
    ...
    facts: [
      ...
      { label: "Money in", value: inKwacha ? "Kwacha (K)" : "Dollars (USD)" },
      {
        label: inKwacha ? "At the rate" : "Rate on file",
        value: rate === null ? "none published" : `USD 1 = K ${rate.toLocaleString("en-US")}`,
      },
  ...
  const cards = (
    ...
    items.forEach((item, i) => {
      ...
      /* The figure shrinks to fit its card rather than running off it: a
         kwacha figure in the tens of millions is a lot of digits. */
      let size = 14;
