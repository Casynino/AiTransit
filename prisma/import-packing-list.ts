...
/** Zambian numbers arrive as 0XXXXXXXXX on these sheets. */
...
/** Bilingual description; the China desk works in Chinese, Lusaka in English. */
  ...
  */
  ...
  if (replace) {
    ...
    const cleared = await prisma.$transaction([
      ...
      // Restart document numbering so the imported cargo reads AT-000001 up.
  ...
  for (const line of parsed) {
    ...
    const trackingNumber = `AT-${pad(++shipmentSeq)}`;
    ...
    const shipment = await prisma.shipment.create({
      data: {
        ...
        /*
          ...
          sits beside it. A Lusaka clerk reading English sees "Auto parts"; the
