...
import { ALL_PRODUCTS } from "./price-list";

const prisma = new PrismaClient();

/** Every product the published price list actually contains. */
const CANONICAL = new Set(
  ALL_PRODUCTS.map((product) => `${product.category}::${product.name}`)
);
...
const NEGATIVE: Record<string, string[]> = {
  /* An accessory is not the thing it is an accessory for, and each contains the
     other's name. AITRANSIT prices everything by weight, so getting this wrong
     is no longer the difference between USD 25 and USD 3 — but it still puts
     cargo in the wrong CATEGORY, which decides the rate and the airport. A
     phone case is normal goods out of Guangzhou; a phone is special category
     out of Hong Kong. */
  "Phones & accessories": ["phone case", "phone cover", "手机壳", "手机套"],
  "Cameras & audio": ["camera bag", "相机包", "speaker stand"],
  "Laptops & computers": ["laptop bag", "laptop stand", "电脑包", "电脑支架"],
  Wigs: ["wig stand", "wig cap", "wig net"],
};
