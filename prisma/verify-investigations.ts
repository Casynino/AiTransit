...
async function main() {
  ...
  check(
    ...
    restoredStatus("READY_FOR_PICKUP", "RECEIVED_AT_ZAMBIA") === "READY_FOR_PICKUP"
  );
  check(
    "cargo missing at check-in comes back as received, not in transit",
    restoredStatus("IN_TRANSIT", "IN_TRANSIT") === "RECEIVED_AT_ZAMBIA",
    "a found box is on the Lusaka floor; IN_TRANSIT would hide it from Inventory"
  );
  check(
    "cargo that never left China still comes back as received in Lusaka",
    restoredStatus("READY_TO_DEPART", "IN_TRANSIT") === "RECEIVED_AT_ZAMBIA"
  );
  check(
    "no history at all falls back to the shipment's own status, clamped",
    restoredStatus(null, "READY_FOR_PICKUP") === "READY_FOR_PICKUP" &&
      restoredStatus(null, "IN_TRANSIT") === "RECEIVED_AT_ZAMBIA"
  );
  check(
    "restore never returns a terminal status",
    (["DELIVERED", "CANCELLED"] as const).every(
      (status) => restoredStatus(status, status) === "RECEIVED_AT_ZAMBIA"
    )
  );
