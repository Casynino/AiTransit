...
const CITIES: City[] = [
  ...
  { id: "dar", name: "Lusaka", x: 520, destination: true },
];

const ZAMBIA = { x: 520, y: onLimb(520) };

/**
 * Arcs that climb off the surface and come back down to Lusaka.
 *
...
const ROUTES = CITIES.filter((c) => !c.destination).map((city, i) => {
  ...
  const mid = (from.x + ZAMBIA.x) / 2;
  return {
    id: city.id,
    d:
      `M${from.x} ${from.y.toFixed(0)} ` +
      `Q${mid.toFixed(0)} ${(Math.min(from.y, ZAMBIA.y) - lift).toFixed(0)} ` +
      `${ZAMBIA.x} ${ZAMBIA.y.toFixed(0)}`,
