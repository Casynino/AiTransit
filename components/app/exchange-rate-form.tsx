 ...
 * spot as "ZMW 2,700,000 for a hundred dollars" than as a number in a box.
 */
    ...
    <form action={action} className="space-y-4">
      ...
      {state?.ok ? (
        <p className="rounded-md border border-success/30 bg-success/5 p-3 text-sm text-success">
          {t("Rate published")}
          {state.data ? `: 1 USD = ${state.data.rate.toLocaleString()} ZMW` : ""}.{" "}
          {t("Invoices already raised keep the rate they were raised at.")}
        </p>
      ) : null}

      <div className="space-y-1.5">
        <Label htmlFor="rate">{t("New rate — kwacha to one US dollar")}</Label>
        <Input
          id="rate"
          name="rate"
          inputMode="decimal"
        ...
        <div className="rounded-lg border bg-muted/40 p-3 text-sm">
          <div className="flex items-baseline justify-between gap-3">
            ...
            <span className="font-mono font-semibold tabular-nums">
              ZMW {Math.round(value * 100).toLocaleString()}
            </span>
          </div>
          {move !== null ? (
