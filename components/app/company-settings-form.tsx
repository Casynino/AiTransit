...
export function CompanySettingsForm({ initial }: { initial: CompanySettings }) {
  ...
  const setOffice = (
    which: "zambia" | "china",
    patch: Partial<CompanySettings["zambia"]>
  ...
  return (
    <form action={action} className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {/*
          The one label the business has not settled on.

          Its own block rather than a line among the addresses, because it is
          the only field on this page that changes what STAFF see in a dropdown
          rather than what a customer reads on an invoice — and the note has to
          be next to it, so nobody renames it expecting prices to follow.
        */}
        <section className="rounded-xl border bg-card p-5">
          <h2 className="font-display text-lg font-semibold">
            {t("Cargo category name")}
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t(
              "What the higher-rate category is called on screen and on invoices. Renaming it changes the words only — every rate, rule and priced invoice is unaffected."
            )}
          </p>
          <input
            value={settings.specialCategoryName}
            onChange={(e) =>
              setSettings((s) => ({ ...s, specialCategoryName: e.target.value }))
            }
            className="mt-4 h-11 w-full max-w-sm rounded-lg border bg-background px-3 text-sm"
            maxLength={40}
          />
        </section>

        {(["zambia", "china"] as const).map((which) => (
          <section key={which} className="panel p-5">
            <h2 className="font-display font-semibold">
              {which === "zambia" ? t("Zambia office") : t("China office")}
            </h2>
            <p className="mt-0.5 text-xs text-muted-foreground">
              {t(
