...
export type PayrollLineRow = {
  ...
  /** USD, like every stored figure here. The screen prints kwacha. */
  gross: number;
  allowance: number;
  deduction: number;
  net: number;
...
function EditableLine({
  ...
  return (
    <form action={save} className="flex flex-wrap items-center gap-2 px-3 py-1.5">
      ...
      {/* Typed in dollars, which is what the line is stored in. A kwacha box
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 px-3 py-1.5">
      ...
      {/* A dash where there is nothing. Most people have neither an allowance
          nor a deduction, and thirty stacked "K 0" would bury the two lines
          that do. */}
 ...
 * admin is being asked to agree to: money leaving a named account whose
...
export function PayrollSubmit({
  ...
  /* A dollar bill cannot leave a kwacha account with no rate published, and
     the action refuses it at payment. Saying so now spares the admin agreeing
  ...
  return (
    <form action={submit} className="space-y-2 rounded-xl border bg-card p-3">
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <div className="min-w-[11rem] flex-1 space-y-0.5">
          <label
            ...
            className="block text-[11px] font-medium text-muted-foreground"
          >
            {t("Anything the admin should know")}
          </label>
          <Input
      ...
      <p className="text-[11px] text-muted-foreground">
        ...
        {t(
          "Nothing on the run can be changed once it has gone up. It comes back only if the admin sends it back."
        )}
      </p>
      ...
      {needsRate ? (
        <p className="text-[11px] font-semibold text-warning">
          {t(
            "No exchange rate is published, so a dollar salary bill cannot be paid out of a kwacha account. Publish one before this is agreed."
          )}
        </p>
      ) : null}
