  ...
  return (
    <DocumentSheet>
      <DocumentHeader
        ...
        meta={
          <>
            <p className="font-mono text-sm font-bold tabular text-[#103B7C]">
              {invoiceNumber}
            </p>
            <p>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div className="flex items-start justify-between gap-3">
          <DocumentField label={t(locale, "Cargo")}>
            <p className="font-mono text-lg font-bold tabular text-[#103B7C]">
              {shipment.trackingNumber}
            </p>
            <p className="text-xs">
              {shipment.batchNumber ? `${shipment.batchNumber} · ` : ""}
              {t(locale, shipment.originLabel)} — {t(locale, "Lusaka")}
            </p>
          </DocumentField>
      ...
      <table className="mt-6 w-full border-collapse text-sm">
        <thead>
          <tr className="bg-[#103B7C] text-left text-white">
      ...
      {/*
        ...
        person settling it walks into a shop and sends kwacha. Leading with
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-[minmax(0,1fr)_260px]">
        ...
        <div
          className={`rounded-lg p-5 text-white ${
            paidInFull ? "bg-[#117447]" : "bg-[#103B7C]"
          }`}
        >
      ...
      */}
      {paidInFull ? (
        <p className="mt-7 flex flex-wrap items-center justify-between gap-2 rounded-lg bg-[#117447]/10 px-5 py-4 text-sm font-bold text-[#117447]">
          {t(locale, "Settled in full — thank you for your business.")}
          <span className="text-xs font-medium text-black/55">
            {t(locale, "No payment is outstanding on this invoice.")}
          </span>
        </p>
      ) : (
        <section className="mt-7 rounded-lg border-2 border-[#103B7C] p-5">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <h2 className="font-display text-sm font-bold uppercase tracking-wide text-[#103B7C]">
              {t(locale, "How to pay")}
            </h2>
            <p className="text-xs text-black/55">
          ...
          <div className="mt-3 grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2">
            ...
            {accounts.map((account) => (
              <div key={`${account.label}-${account.number}`}>
                ...
                <p className="font-mono text-base font-bold tabular text-[#103B7C]">
                  {account.number}
                </p>
      ...
      {/*
        The storage terms, in English, whatever language the clerk has selected.

        The INTERFACE speaks English and Chinese, because that is what the two
        desks running it speak. This document is not the interface — it is what
        a customer in Lusaka is handed, and they read English. Printing it in
        whichever language the clerk happened to have selected would put a
        Chinese storage warning on a Zambian customer's bill.
      */}
      <section className="mt-5">
        <DocumentField label={t(locale, "Storage policy")}>
          {/* Red on purpose. A storage charge nobody read is the one argument
              this document exists to prevent, and it was set in the same grey
              as the bank details above it. One language now — see the note on
              storageNotice(); AITRANSIT bills into Lusaka in English. */}
          {/* The notice's own heading is not printed — the DocumentField label
              above already says "Storage policy", and with one language the two
              were the same words twice. */}
          <div className="space-y-2 text-xs leading-relaxed text-[#2baee6]">
