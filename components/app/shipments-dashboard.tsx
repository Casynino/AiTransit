...
export type DispatchRow = {
  ...
  money?: {
    ...
    drafts: number;
    /**
     * What clearing this flight has cost, in dollars and in kwacha.
     *
...
/**
 * Money on a board, where the question is scale and not the last kwacha.
 *
...
export function ShipmentsDashboard({
  ...
  /** USD → ZMW. Null when no rate is published; the board falls back to USD. */
  rate: number | null;
  /**
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      spentZmw: sum((r) => r.money?.spentZmw ?? 0),
    };
  }, [visible]);
  ...
  */
  ...
  if (showMoney && showCosts) {
    ...
    cells.push(
      {
        // Kwacha summed at face value where they were paid in kwacha,
        // exactly as a flight's own expenses panel does it.
        k: t("Expenses"),
        main: rate === null ? formatUsd(totals.spentUsd) : formatLocal(totals.spentZmw),
  ...
  return (
    <div>
      {/*
        ...
        label above, kwacha large, dollars small — so a total means the same
      ...
      */}
      ...
      {visible.length === 0 ? (
        ...
        <section className="overflow-hidden rounded-xl border bg-card shadow-soft">
              ...
              <li key={row.id}>
                ...
                >
                  ...
                  {/* Origin and flight only. Every one of these landed in
                      Lusaka, so printing the destination on each row is
          ...
          {/* The unit, said once. Repeating "K" down four columns of a
          ...
          {showMoney ? (
            <p className="border-t px-4 py-1.5 text-right text-[11px] text-muted-foreground">
              {rate === null
                ...
                : t("Money columns in thousands (k) and millions (M) of kwacha")}
            </p>
          ) : null}
        </section>
      )}
