 ...
 *
 * WHY PER ROLE. A Lusaka phone and the CEO's phone are the same build and not the
 * same job. Lusaka lives in Receive and Release, Guangzhou in Receive cargo and
...
const TABS: Record<Role, Tab[]> = {
  ...
  /*
    The one management bar. AITRANSIT folded the second management chair into
    ADMIN, so this role now carries both jobs — and a phone bar cannot show
    eight things. The control room takes the third slot rather than the arrived
    batches: chasing a customer is Finance's job and reading a manifest is a
    desk job, but "is anything waiting on me" is the question this person opens
    their phone to ask, and /app/admin/control is the screen that answers it.

    Everything the two old bars held between them is one tap further, in the
    drawer, where the full menu lives.
  */
  ADMIN: [
    { href: "/app/admin", icon: LayoutDashboard, exact: true },
    { href: "/app/search", icon: Search },
    { href: "/app/admin/control", icon: ShieldCheck, label: "Control" },
  ...
  // The Lusaka floor works a line: a box lands and is checked in, a customer
  ...
  /* A customer never reaches /app at all — the portal at /portal has its own
     navigation. The row exists because Role is exhaustive and an omission here
     would be a type error rather than a considered decision. */
  CUSTOMER: [],
  CUSTOMER_CARE: [
