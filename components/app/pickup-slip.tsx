 ...
 * going to fold it into a pocket, carry it across Lusaka and hold it out at
 * a counter.
 *
...
export async function PickupSlip({ data }: { data: PickupSlipData }) {
  ...
  return (
    ...
    >
      ...
      <span aria-hidden className="absolute inset-x-0 top-0 bg-[#103B7C]" style={{ height: "1.6mm" }} />
      <span aria-hidden className="absolute left-0 top-0 bg-[#2BAEE6]" style={{ height: "1.6mm", width: "26mm" }} />
          ...
          {spent ? (
            <p
              ...
              style={{
                fontSize: "9pt",
                marginTop: "1.2mm",
                color: data.status === "USED" ? "#117447" : "#2BAEE6",
              }}
            >
              {data.status === "USED"
                ? t(locale, "Collected")
