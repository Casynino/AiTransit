...
/**
 * The paper every AITRANSIT document is printed on.
 *
...
export const DOC_NAVY = "#103B7C";
export const DOC_RED = "#2BAEE6";
...
export function DocumentSheet({
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden
        className="absolute inset-x-0 top-0 z-10 h-1.5 bg-[#103B7C]"
      />
      <span aria-hidden className="absolute left-0 top-0 z-10 h-1.5 w-32 bg-[#2BAEE6]" />
...
export function DocumentHeader({
  ...
  return (
    <header className="flex flex-wrap items-start justify-between gap-4 border-b-2 border-[#103B7C] pb-5">
      <div>
        <BrandLogo className="h-14 w-auto" />
        <p className="mt-2.5 text-xs font-semibold uppercase tracking-[0.22em] text-[#103B7C]">
          {title}
        </p>
      </div>
  ...
  return (
    <footer className="mt-7 border-t border-black/20 pt-3 text-xs leading-relaxed text-black/60">
      <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-2">
        <div>
          <p className="text-xs font-bold text-[#103B7C]">{COMPANY.name}</p>
          <p className="font-bold tracking-[0.08em] text-[#2BAEE6]">
