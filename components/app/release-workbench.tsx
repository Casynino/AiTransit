...
/**
 * The Lusaka counter, on one screen.
 *
  ...
  return (
    <div className="panel overflow-hidden">
      ...
      <dl className="grid grid-cols-2 gap-px bg-border sm:grid-cols-3">
        ...
        <Fact label={t("Arrived in Lusaka")} value={target.arrivedAt ?? "—"} />
        <Fact
          label={t("Pickup note")}
      ...
      <div className="space-y-3 border-t p-4 sm:p-6">
        ...
        {/*
          The box actually in the clerk's hand, and whether Lusaka ever receipted
        ...
        */}
        {target.scannedPackage ? (
          ...
          >
            ...
            {target.scannedPackage.received
              ? t("checked in at Lusaka.")
              : t("this one was never checked in at Lusaka.")}
          </p>
        ) : null}
        ...
        */}
        ...
        {note && note.status === "ACTIVE" ? (
          /*
            ...
            so the Lusaka counter reads "settled in full" and the note that says
            ...
            server: the kwacha are simply absent from what the browser is
 ...
 * somewhere else, with a customer watching. The Lusaka floor holds
