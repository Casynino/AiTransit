...
export type WorkItem = {
  ...
  cta: string;
  /**
   * Money at stake, in dollars. Rendered kwacha-first. Omit entirely for a
...
export async function WorkList({
  ...
  empty: string;
  /** USD→ZMW. Null when no rate is published — then figures stay in dollars. */
  rate: number | null;
}) {
              ...
              >
                ...
                <span className="text-right">
                  {item.usd !== undefined && item.usd > 0 ? (
                    <>
                      {/* Kwacha lead. Freight is priced in dollars and paid
