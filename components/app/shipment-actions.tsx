...
export function ShipmentActions(props: Props) {
  ...
  */
  ...
  const canIssueNote =
    can(role, "pickupNote.issue") && status === "RECEIVED_AT_ZAMBIA" && releasable;
  /**
...
function InvoicePanel(props: Props) {
  ...
  const arrived =
    props.status === "RECEIVED_AT_ZAMBIA" ||
  ...
  // in at Lusaka, so reaching this means the rate book could not price it or the
  ...
  return (
    <div className="p-5">
      {/*
        ...
        The figure comes from the weight and piece count the Lusaka floor
      ...
      */}
      {!arrived ? (
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            {t(
              "This cargo has not been checked in at Lusaka yet, so there is no final weight to price it on. The bill is raised by the system the moment the warehouse checks it off the manifest."
            )}
          </p>
        </div>
      ) : (
...
function PaymentPanel(props: Props) {
  ...
  // Kwacha by default. Bills are quoted in dollars because that is what the
  // rate book is in, but almost every customer pays in kwacha at the
  ...
  // currency, so kwacha cannot land in the dollar account; and cash goes in
      ...
      : !rateUsable
        ...
        : Number.isFinite(typed) && typed > 0
          ? `${currency} ${typed.toLocaleString()} ${t("settles")} ${props.currency} ${(
              currency === "ZMW" ? typed / activeRate : typed * activeRate
            ).toFixed(2)} ${t("at")} ${activeRate.toLocaleString()}.`
          : null;

  // Agreeing a different rate changes what the same kwacha are worth, so a
      ...
      {open && !settled ? (
        <form action={action} className="mt-4 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                {props.currency !== "ZMW" ? <option value="ZMW">ZMW</option> : null}
          ...
          {/* Where the money landed.
              ...
              have received this money: a kwacha account cannot hold dollars,
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              {t(
                "The mobile money screenshot, bank slip or transfer confirmation. This is what settles an argument months later — a typed number is only a claim that it happened."
              )}
            </p>
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              id="reference"
              name="reference"
              placeholder={t("Mobile money ID, slip or cheque number")}
            />
            <p className="text-xs text-muted-foreground">
              {t(
    ...
    <div className="p-5">
      <form action={action} className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          {t(
            blocked
              ? "Available once the cargo is checked in at Lusaka and the invoice is settled in full."
              : onCredit
