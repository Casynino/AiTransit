...
const money = (n: number, currency: string) =>
  `${currency === "USD" ? "USD" : "K"} ${n.toLocaleString("en-US", {
    maximumFractionDigits: currency === "USD" ? 2 : 0,
  })}`;

/**
 * A bill in kwacha, with the dollars underneath.
 *
 * The owner's rule, and it is about which currency this business actually runs
 * on: pricing is done in dollars, but customers pay in kwacha and the office
 * counts in kwacha. So every figure leads in K and carries the USD it was
 * billed at — never the other way round, which made a clerk convert in their
 * head before the number meant anything.
 */
function inKwacha(usd: number, rate: number | null) {
  return rate === null ? null : `K ${Math.round(usd * rate).toLocaleString("en-US")}`;
}
 ...
 *
 * Income here is never free text. Every kwacha this business earns is earned
...
export function RecordIncome({
  ...
  /** Today's USD→ZMW, shown so nobody has to guess what a dollar bill is worth. */
  rate: number | null;
  /**
   * Open on arrival.
   *
  ...
  /*
    ...
    currency of the bill: a USD invoice is routinely settled in kwacha at
    the counter.
    ...
    rightly — so offering the bank account (ZMW) for a payment marked USD is offering a
    choice that can only end in an error message. The list follows the switch.
  */
  const [tendered, setTendered] = useState<string>("ZMW");
  ...
  */
  ...
  /*
    ...
    It matters for more than display. The action converts a kwacha payment at
    the invoice's rate, so a kwacha amount worked out at today's rate would
    credit slightly more or less than the balance and leave a bill a few
    hundred kwacha short of settled. Converting at the same rate the action
  ...
  */
  ...
  /*
    Kwacha by default, whatever the bill says.

    Pricing is in dollars; paying is not. A customer settling a USD bill hands
    over kwacha at the counter almost every time, so the form opens on what
    is actually about to happen — and the amount opens as the kwacha value of
                        ...
                        {settled ? (
                          ...
                          <span className="text-right">
                            <span className="block text-sm font-semibold tabular-nums text-destructive">
                              {hit.currency === "ZMW"
                                ? money(hit.outstanding, "ZMW")
                                : (inKwacha(hit.outstanding, hit.rate ?? rate) ??
        ...
        <form action={action} className="px-5 py-4">
          ...
          <div className="mb-3 flex flex-wrap items-baseline gap-x-3 gap-y-1 rounded-lg border bg-card px-4 py-2.5">
            ...
            <span className="text-xs text-muted-foreground">
              ...
              <span className="font-semibold tabular-nums text-destructive">
                {picked.currency === "ZMW"
                  ? money(picked.outstanding, "ZMW")
                  : (inKwacha(picked.outstanding, billRate) ??
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            {/*
              ...
              mobile money code went with it: a photograph of the message says more
