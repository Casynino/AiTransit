  ...
  if (state?.ok) {
    return (
      <p className="rounded-lg border border-success/30 bg-success/5 p-2.5 text-xs text-success">
        {invoiceNumber} {t("is cancelled. The cargo will be priced again when Lusaka checks it in.")}
      </p>
    );
  }
  ...
  return (
    ...
    >
      ...
      <p className="text-[11px] text-muted-foreground">
        {confirmed
          ? t(
              "The bill is removed and the cargo goes back to having no price. It will be priced again from the rate book when Lusaka checks it in. Who cancelled it, and why, stays on the audit log."
            )
          : t(
              "This price was never signed off, so nobody has been asked for it. The cargo goes back to having no bill and will be priced again when Lusaka checks it in."
            )}
      </p>
