 ...
 * bill is written in.
 *
 * KWACHA LEAD. This is a Zambian business: customers hand over kwacha,
 * the desk counts kwacha, and the question "have we got the money" is asked
 * in kwacha. The rate book and the invoice are in dollars because that is
...
export async function MoneyTile({
  ...
  label: string;
  usd: number;
  /** Live USD→ZMW rate. Null when none is published — then no conversion shows. */
  rate: number | null;
  hint?: string;
  icon?: LucideIcon;
  tone?: Tone;
   ...
   * kwacha — the revenue tile was a KpiCard purely because this was missing,
   * and it was the one money card on the page still reading in dollars.
   */
  trend?: number[];
}) {
  const locale = await viewerLocale();
  /* Digits, not dollars: the kwacha figure is what has to fit. */
  ...
  /*
    ...
    tabular digit is about 0.6em, and "K " with its margin is about 1.8 —
  ...
  const body = (
    ...
    >
      ...
      {/*
        ...
        laptop — and "K 38,486,610" at 30px needs more than that. It did not
        ...
        Kwacha run long by nature — a million-dollar business prints ten
        ...
        {rate ? (
          <>
            {/* Half the size of the figure and in its colour, not muted grey.
                This is a Zambian business — the unit is not a footnote to the
            ...
            <span className="mr-1 text-[0.63em] font-bold opacity-80">K</span>
            {figure}
          </>
        ) : (
          formatUsd(usd)
