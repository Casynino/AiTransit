 ...
 * transaction as the acceptance, so there is no state in which the admin
 ...
 * closes on the spot. Nothing else on either payroll screen moves a kwacha,
 * so this is the one control that has to be impossible to press by accident.
 *
 * A kwacha account with no published rate is refused up front rather than
 * left to fail in the action — there is nothing the admin could do here to
    ...
    >
      ...
      {needsRate ? (
        <p className="text-[11px] font-semibold text-destructive">
          {t(
            "No exchange rate is published, so a dollar salary bill cannot leave a kwacha account. Publish one on Pricing & configuration first."
          )}
        </p>
      ) : null}
