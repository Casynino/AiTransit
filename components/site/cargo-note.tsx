 ...
 *
 * A headline and a detail line, both in English: this is written for the
 * person collecting the box, and they are in Lusaka.
 */
...
export function CargoNote({
  ...
  /** How many of the customer's boxes have been checked in at Lusaka, if known. */
  ...
  const arrived = stampFor(timeline, "RECEIVED_AT_ZAMBIA");
  ...
  if (hold) {
    ...
    if (hold.state === "COMPENSATION_IN_PROGRESS") {
      headline = "We are handling your claim.";
      en =
      ...
      headline = "Landed — we are checking it.";
      en =
        `It landed on ${formatDate(arrived)} and is here in Lusaka. ` +
        `We opened a check on it ${formatRelative(hold.since)} and it stays with us until that is settled.`;
    } else if (departed) {
      headline = "Not yet checked in at Lusaka.";
      en =
        `It left ${origin} on ${formatDate(departed)} and has not been checked in at our Lusaka warehouse. ` +
        `We have been tracing it since ${formatDate(hold.since)} and will call you the moment we know more.`;
    } else {
      headline = "Still in China, being checked.";
      en =
    ...
    icon = PackageCheck;
    headline = "Collected. Thank you for shipping with us.";
    en = `Collected on ${formatDate(delivered)} — ${packagesLabel}, handed over and signed for. Thank you.`;
  } else if (status === "READY_FOR_PICKUP") {
    icon = PackageCheck;
    headline = "Ready for collection.";
    en =
      `Cleared for collection ${ready ? formatRelative(ready) : "today"}. ` +
      `Bring this tracking number to our Makeni warehouse — storage is free for ` +
      `${STORAGE_POLICY.freeDays} days from the day it landed.`;
  } else if (status === "RECEIVED_AT_ZAMBIA") {
    icon = PlaneLanding;
    headline = "Arrived safely in Lusaka.";
    en =
