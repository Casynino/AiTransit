...
import { formatKwacha, formatUsd, toLocal } from "@/lib/money";
import {
  trackByCode,
  type PublicCharge,
...
export const metadata: Metadata = {
  ...
  description:
    "Enter your AITRANSIT tracking number or batch number to see where your cargo is.",
};
      ...
      <PageHero
        ...
        body={
          <>
            ...
            <span className="font-mono text-white">AT-000123</span>) or the batch
            number we gave you (
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground">
            ...
            <a
              href={`https://wa.me/${COMPANY.whatsapp}?text=${encodeURIComponent(
                `Hello AITRANSIT, I am asking about cargo ${trackingNumber}`
              )}`}
              target="_blank"
      ...
      */}
        ...
        >
          ...
          <dl className="grid grid-cols-2 gap-px bg-border sm:grid-cols-4">
            {[
              ...
              {
                /* The accrued charge leads in kwacha, like the bill below —
                ...
                v: formatKwacha(result.storage.chargeUsd, result.publishedRate),
                sub:
      ...
      <dl className="grid grid-cols-1 gap-px bg-border sm:grid-cols-3 lg:grid-cols-4">
        {[
          ...
          { label: "Route", value: `${result.origin} → Lusaka` },
      ...
      {result.estimate && !result.charge ? (
        <div className="border-t bg-muted/30 p-6">
          ...
          {/*
            Kwacha lead for the same reason they lead on the bill below: it
          ...
          {result.publishedRate !== null && result.estimate.currency === "USD" ? (
            <>
              <p className="mt-2 font-display text-3xl font-bold tabular">
                <span className="mr-1.5 text-xl font-bold opacity-70">K</span>
                {toLocal(
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t bg-muted/30 p-5 text-xs text-muted-foreground">
        ...
        <a
          href={`https://wa.me/${COMPANY.whatsapp}?text=${encodeURIComponent(
            `Hello AITRANSIT, I am asking about cargo ${result.trackingNumber}`
          )}`}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 font-medium text-foreground hover:text-brand"
 ...
 *
 * Shows the kwacha figure at the invoice's own rate — the number they were
      ...
      <div className="border-t bg-muted/30 p-5 text-sm text-muted-foreground">
        ...
        {held ? null : (
          <p className="mt-1 text-xs">
            Your invoice is raised once the cargo is checked in at Lusaka es
            Salaam.
          </p>
        )}
      </div>
    ...
    <div className="border-t">
      <div className="flex flex-wrap items-start justify-between gap-6 p-6">
        <div>
          ...
          {/*
            Kwacha lead, dollars underneath.

            This is the one figure on the public site a customer acts on, and
            they pay it in kwacha — at a Lipa number, in a bank hall, in
            cash at the counter in Makeni. Leading in dollars asked every
          ...
          */}
          {(settled ? charge.totalLocal : charge.outstandingLocal) !== null ? (
            ...
            /* No rate on the invoice yet, so there is no honest kwacha
            ...
            <dl className="mt-4 space-y-1.5 border-t pt-3 text-sm">
              ...
              {charge.exchangeRate ? (
                <div className="flex justify-between gap-4 text-xs text-muted-foreground">
                  ...
                  <dd className="font-mono tabular-nums">
                    USD 1 = {charge.localCurrency ?? "ZMW"}{" "}
      ...
      {!settled ? (
        <div className="mt-4 rounded-lg border bg-muted/20 p-4">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            ...
            as the payment reference. Once you have paid, send us the proof on{" "}
            <a
              href={`https://wa.me/${COMPANY.whatsapp}`}
              target="_blank"
              rel="noreferrer"
