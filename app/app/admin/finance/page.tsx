...
/**
 * The admin's money screen: the period, the P&L behind it, and the flights.
 *
 ...
 * tidiness: a admin's page with its own revenue query would disagree with the
...
export default async function AdminFinance({
  ...
  */
  ...
  const sortHref = (key: string) => {
    ...
    return `/app/admin/finance?${params.toString()}`;
  };
    ...
    <>
      ...
      <div className="mb-2 flex flex-wrap gap-1.5">
        {PERIODS.map((p) => (
          <Link
            key={p.key}
            href={`/app/admin/finance?period=${p.key}`}
      ...
      <p className="mb-3 text-[11px] text-muted-foreground">
        ...
        {rate === null
          ...
          : ` · USD 1 = K ${rate.toLocaleString("en-US")}`}
      </p>
