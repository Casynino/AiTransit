...
import { formatLocal, formatKwacha, formatUsd } from "@/lib/money";
...
/**
 * The admin's batch control desk: every flight's figures, the costs behind
 ...
 * /app/admin/finance reads both. A control page with its own arithmetic
 ...
 *
 * AND IT EDITS NO FINANCIAL RECORD. The owner's rule: a admin disputing a
 * figure must not touch the figure. A verdict here is one appended
 * RecordReview row kept BESIDE the batch — Finance still sees exactly what
 * was recorded, plus the fact that a admin disputes it and why. The desk
 ...
 * money actions use: nothing is being claimed. Two admins flagging the same
...
async function reviewBatch(formData: FormData) {
  ...
  if (!batchId || !(verdict in VERDICTS)) redirect("/app/admin/batches");
  ...
  if (!reason) {
    redirect(`/app/admin/batches?note=reason&batch=${batchId}#b-${batchId}`);
  }
  ...
  if (!batch) redirect("/app/admin/batches");

  await prisma.recordReview.create({
    data: {
      target: "BATCH",
      targetId: batchId,
  ...
  redirect(`/app/admin/batches?note=recorded&batch=${batchId}#b-${batchId}`);
}
...
export default async function AdminBatches({
  ...
  */
  ...
  const sortHref = (key: string) => {
    ...
    return `/app/admin/batches?${params.toString()}`;
  };
  ...
  const openHref = (id: string) => {
    ...
    params.set("batch", id);
    return `/app/admin/batches?${params.toString()}#b-${id}`;
  };
      ...
      {sorted.length === 0 ? (
        ...
        <>
          {/*
            ...
            caller (app/app/admin/finance) already wrote down: that is a
