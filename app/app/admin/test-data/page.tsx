 ...
 * — so it is gated behind user.manage (the CEO and the admin). It exists because a QR
...
export default async function TestDataPage() {
  ...
  const unpaidAtZambia = shipments.find(
    (s) => s.status === "RECEIVED_AT_ZAMBIA" && s.invoice?.status !== "PAID"
  );
  ...
  const scenarios = [
    {
      ...
      steps: [
        ...
        t(
          locale,
          "The verdict changes with your role: Finance sees the money, the Lusaka warehouse sees whether it may release the cargo."
        ),
    ...
    },
    {
      ...
      steps: unpaidAtZambia
        ? [
            `${t(locale, "Open")} ${unpaidAtZambia.trackingNumber} — ${t(locale, "it is in the Lusaka warehouse with an unpaid invoice.")}`,
            t(
              locale,
    ...
    {
      icon: Truck,
      role: t(locale, "Lusaka Warehouse"),
    ...
    },
    {
      ...
      steps: [
        t(
          locale,
          "Go to Receive cargo and type 0977111222 in the phone field — it recognises Makeni Traders and fills the name for you."
        ),
        t(
          locale,
    ...
    },
    {
      ...
      steps: [
        `${t(locale, "Open the public site and search")} ${inTransit?.trackingNumber ?? readyToDepart?.trackingNumber ?? "AT-000005"} ${t(locale, "on the tracking page.")}`,
        t(
          locale,
