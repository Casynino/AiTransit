...
const ASKS: Partial<Record<ReportKey, string>> = {
  ...
  "mobile-money": "Every movement through Airtel Money, MTN MoMo and the rest.",
...
const SHELVES: { name: string; keys: ReportKey[] }[] = [
  ...
  /* The money desk is a second line of business, not a corner of the cargo one,
     so it gets its own shelf rather than being read as an "other" report. */
  {
    name: "Money exchange and China payments",
    keys: ["exchange-requests", "supplier-payments"],
  },
];
 ...
 * beside each other that a admin is asked about in the same breath and
...
export default async function AdminReport({
  ...
  const money = (n: number) => formatKwacha(n, rate);
  ...
  /* Against the period before it, because a figure with nothing beside it is
     ...
     same call the admin's own overview strip makes, so a change quoted off
  ...
  const rows: {
    ...
    },
    {
      ...
      /* Only against a profit. percentDelta divides by whatever base it is
         ...
         large NEGATIVE change — the one figure on this sheet a admin could
         act on backwards. */
      delta:
        previous.profit > 0
  ...
  /*
    The endpoint Finance already downloads from, deliberately not a admin copy
    of it. The register, the runner, the CSV and the PDF are one path, so a
    figure the admin hands over and a figure Finance hands over cannot differ.

    `period` carries the pills above onto the document — the route resolves it
    through the same windowFor this page used — and `unit=zmw` matches the
    screen, which leads in kwacha. The handful of reports that can only be
  ...
  */
  const download = (key: ReportKey, format?: "pdf") =>
    `/api/finance/reports?report=${key}&period=${picked.key}&unit=zmw${
    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap gap-1.5 print:hidden">
        {PERIODS.map((p) => (
          <Link
            key={p.key}
            href={`/app/admin/reports?period=${p.key}`}
            className={
              picked.key === p.key
