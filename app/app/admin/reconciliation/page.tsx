...
/**
 * THE ADMIN'S CONTROL CENTRE, NOT A REPORT.
 *
 * The owner's instruction, in full: "THE ADMIN MUST BE ABLE TO DO THE WORK
 ...
 * or a batch Finance already recorded. A verdict is an append-only RecordReview
...
function withParams(params: Params, changes: Params) {
  ...
  return `/app/admin/reconciliation${query ? `?${query}` : ""}`;
}
...
export default async function AdminReconciliation({
  ...
  const kwacha = (usd: number) => formatKwacha(usd, rate);
    ...
    <>
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <a
              href={`/app/admin/reconciliation/export${
                new URLSearchParams(
      ...
      */}
      ...
      <section className="mb-4">
        <form
          action="/app/admin/reconciliation"
      ...
      */}
      <section className="mb-6 grid grid-cols-1 items-stretch gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] xl:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)]">
        ...
        */}
        <div className="flex flex-col overflow-hidden rounded-xl border bg-card shadow-soft">
          <div className="border-b px-4 py-3">
            ...
            {/*
              ...
              him — "this week, the bank account, still pending" states its own size.
            */}
          ...
          {/*
            ...
            thing the server cannot: which rows the admin has picked.
          */}
          <RecordsQueue
            canReview={canReview}
            emptyLabel={
        ...
        {/* the record under the admin's eye */}
              ...
              <div className="px-4 py-3">
                ...
                {canReview ? (
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t(locale, "Reading only — recording a verdict is the admin's and the owner's.")}
                  </p>
                )}
              </div>
                ...
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <p className="text-sm font-semibold">
                    ...
                    <span className="ml-2 text-xs font-normal text-muted-foreground">
                      {kwacha(selectedBatch.revenue)} {t(locale, "billed")} ·{" "}
                      {kwacha(selectedBatch.collected)} {t(locale, "collected")} ·{" "}
                      {kwacha(selectedBatch.outstanding)} {t(locale, "still owed")} ·{" "}
                    {kwacha(selectedBatch.costs)} {t(locale, "cost")}
                    </span>
                  </p>
            ...
            <ul className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              ...
              */}
              {[...checks.checks]
                ...
                .map((check) => {
                  /* In kwacha when the side IS money — the engine writes its
                  ...
                  const sideText = (side: CheckSide) =>
                    side.usd !== undefined ? kwacha(side.usd) : side.value;
                  const gap = money
                    ? kwacha(Math.abs(check.difference))
