 ...
 * meant "eleven payments · K 4,100,000" beside a count of eleven read as all
 * eleven while silently omitting every kwacha claim, in the currency most
 ...
 * rate it used. When a kwacha claim is waiting and no rate has ever been
 ...
 * taken, with the evidence attached to it. A admin who cannot rule on one of
...
export default async function AdminApprovals({
  ...
  /* Items, not queues. "Four queues" is a fact about this page's layout; what
     the admin is being told is how many things are sitting. */
        ...
        {tabs.map((option) => (
          <Link
            ...
            href={
              option.key === "waiting"
                ? "/app/admin/approvals"
                : `/app/admin/approvals?view=${option.key}`
            }
