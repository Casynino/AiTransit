...
import {
  ...
  QueueList,
  StatRows,
} from "@/components/app/admin-bento";
...
import { adminOverview, type Insight, type InsightTone } from "@/lib/admin-overview";
 ...
 *
 * Matched on the insight id from lib/admin-overview.ts, which is a plain
 ...
 *
 * THE SAME BUSINESS THE OWNER READS, AND NOT THE SAME PAGE. A admin who runs
...
export default async function AdminHome() {
  ...
  */
  ...
  /*
    ...
    on this screen is restated in kwacha, and this one deliberately is not:
  ...
  */
  ...
  // Kwacha lead, everywhere, through the one helper — a per-cell `rate ? … :
  // …` is how one panel on a page ends up quoting a different currency from the
  // panel beside it.
  const money = (usd: number) => formatKwacha(usd, rate);
  /*
    Already kwacha — print, do not convert.

    The account figures arrive valued in lib/admin-overview.ts, where each
    account's own currency is known. Running them through `money` a second time
    is what printed "K -29,382,118,200" on this page: a kwacha balance
    multiplied by the rate again. The Zmw suffix on those fields is the only
    thing standing between the two helpers, so it is worth the noise.
  */
  const kwacha = (zmw: number) =>
    `K ${Math.round(zmw).toLocaleString("en-US")}`;
  const count = (n: number) => n.toLocaleString("en-US");

  /*
    What was billed this month, derived rather than queried.

    profitAndLoss defines profit as revenue less costs and adminOverview
  ...
  */
  ...
  /*
    ...
    are different facts, and "in profit by K 0" under an up-arrow states the
  ...
  return (
    <>
      ...
      {/*
        ...
        and this one did not, which left the admin reaching into the sidebar
        for the jobs he does most. The owner, pointing at the Guangzhou desk's
        row: "admin should have these as well but that relate to his account."
        ...
        to take out — the same reason the Lusaka floor has no "Find cargo" pill
      ...
      */}
      ...
      {/*
        ...
        situations — the admin needs to know what is really going on."
        ...
        admin's screen was four hundred pixels down. He was being shown his
      ...
      */}
      <div className="mb-6">
        <AttentionCenter
          items={attention}
          reviewAll={{ href: "/app/admin/control", label: t(locale, "Control room") }}
      ...
      <section className="mb-7">
        ...
        */}
        {/*
          ...
          holding "Collected today · K 0" was stretched to the height of its
          ...
          <div className="mt-4 grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1.75fr)_minmax(0,1fr)] lg:gap-6">
            ...
            <div className="flex flex-col justify-between gap-4 border-t pt-4 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
              ...
              {/*
                ...
                till or a phone since the money last moved — the admin's own
      ...
      */}
      ...
      <section className="mb-7">
        <BandHeading
          ...
          action={{ href: "/app/admin/batches", label: t(locale, "Every batch") }}
        />
          ...
          >
            ...
            <CorridorBar
              ...
              segments={[
                ...
                {
                  key: "floor",
                  label: t(locale, "On the Lusaka floor"),
      ...
      <section className="mb-7">
        <BandHeading
          ...
          action={{ href: "/app/admin/approvals", label: t(locale, "Every queue") }}
        />
        ...
        */}
        <div className="mt-3 flex flex-col gap-2 rounded-xl border bg-card px-4 py-3 sm:flex-row sm:items-center">
          ...
          {/*
            ...
            a bus is counted in it. A admin who read this as attendance would
 ...
 *
 * A dashboard states figures; a admin needs the reading of them. These are
