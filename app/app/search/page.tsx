...
const HANDLES = [
  ...
  },
  {
    ...
    hint: "AT-000125, or the carton reference from the Guangzhou packing list.",
  },
  {
    icon: PackageSearch,
  ...
  },
  {
    ...
    hint: "Typed the way they read it off the handset — 0977…, +260977… or the last digits.",
  },
];
 ...
 * description, customer name, customer code, every shape of a Zambian phone
 ...
 * top of the results.
 *
 * Prices are gated on `finance.view`, so the Lusaka warehouse sees where a box is
...
export default async function SearchCargoPage({
  ...
  /*
    ...
    common Lusaka name that matched a hundred and forty consignments announced "60
      ...
      {query && looksLikeLabel && !scanned ? (
        <div className="mb-4 rounded-xl border border-destructive/40 bg-destructive/5 p-4">
          <p className="flex items-center gap-2 text-sm font-medium text-destructive">
            ...
            {t(locale, "That code is not a AITRANSIT label")}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            {t(
              locale,
            ...
            {scanned.package
              ? scanned.package.deliveredAt
                ...
                : scanned.package.receivedAt
                  ? t(locale, "This box has been checked in at Lusaka.")
