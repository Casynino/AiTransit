...
export default async function ShipmentsPage() {
  ...
  */
  ...
  /*
    ...
    Not a groupBy: a cost paid in kwacha counts at face value and a cost
    ...
    flight is worth reads it in kwacha.
  */
  ...
  const spend = new Map<string, { usd: number; zmw: number }>();
  for (const cost of costs) {
    if (!cost.batchId) continue;
    const usd = toNumber(cost.amountUsd);
    const entry = spend.get(cost.batchId) ?? { usd: 0, zmw: 0 };
    entry.usd += usd;
    entry.zmw +=
      cost.currency === "ZMW" ? toNumber(cost.amount) : rate ? usd * rate : 0;
      ...
      ? {
          ...
          ...(showCosts
            ? {
                ...
                spentZmw: spend.get(dispatch.id)?.zmw ?? 0,
              }
            : {}),
        }
      : null,
