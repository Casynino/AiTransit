...
import { currentRate, formatKwacha, formatUsd } from "@/lib/fx";
...
export default async function FinanceReportsPage({
  ...
  */
  ...
  /*
    ...
    to lead in kwacha; the profit page had not, which is why the answer to
  ...
  */
  ...
  const money = (amount: number) =>
    inUsd ? formatUsd(amount) : formatKwacha(amount, rate);
  /** The same figure in the other currency — null when there is no rate. */
  const alt = (amount: number) =>
    rate === null ? null : inUsd ? formatKwacha(amount, rate) : formatUsd(amount);
    ...
    <>
      ...
      <div className="mb-1.5 flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
        ...
        {/*
          ...
          Kwacha by default, because that is the money the office holds and
      ...
      <p className="mb-4 text-[11px] text-muted-foreground">
        {rate === null
          ? t(
              locale,
              "No exchange rate has been published, so every figure is shown in dollars. Publish one on the Accounts tab and this page reads in kwacha."
            )
          : `${t(locale, inUsd ? "Reading in dollars" : "Reading in kwacha")} · USD 1 = K ${rate.toLocaleString("en-US")} · ${t(locale, "in force since")} ${formatDate(rateRow?.effectiveFrom, locale)}${
      ...
      */}
      ...
      <p className="mb-2 text-[11px] text-muted-foreground">
        {rate === null
          ? t(locale, "In dollars.")
          : inUsd
            ? t(locale, "In dollars. Switch to kwacha at the top of the page.")
            : `${t(locale, "In kwacha, converted at")} USD 1 = K ${rate.toLocaleString("en-US")}. ${t(locale, "Switch to dollars at the top of the page.")}`}
      </p>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="min-w-0 max-w-xl">
            ...
            <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
              ...
              {rate === null
                ? ""
                : ` ${t(locale, "Written in")} ${inUsd ? t(locale, "dollars") : t(locale, "kwacha")}, ${t(locale, "to match the switch at the top of this page.")}`}
            </p>
          </div>
          <form
            method="get"
          ...
          >
            ...
            {inUsd ? null : <input type="hidden" name="unit" value="zmw" />}
