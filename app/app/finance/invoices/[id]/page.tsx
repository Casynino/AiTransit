...
export default async function InvoicePage({
  ...
  // kwacha figure has to keep seeing that figure.
  ...
  */
  ...
  /*
    ...
    lib/auto-price raises every invoice as a DRAFT at Lusaka check-in, so this is
  ...
  */
  ...
  const freightNote = [
    `${t(locale, AIRPORT_LABELS[shipment.origin])} — ${t(locale, "Lusaka")}`,
    ...
    <div className="mx-auto max-w-3xl">
      <div className="no-print mb-6 flex flex-wrap items-center justify-between gap-3">
        {/*
          ...
          "Collections". Saying AT-000125 is the one thing the guess cannot do,
          ...
          <MessageComposer
            ...
            templates={(
              ...
              body: composeMessage(kind, {
                customerName: invoice.customer.name,
                trackingNumber: shipment.trackingNumber,
                // The message goes to a Zambian customer in English, so the
                // cargo line follows the CUSTOMER, not whoever is composing it.
                // A Guangzhou clerk sending this must not send 手机配件 to Lusaka.
