 ...
 *    not balances. There is money in the bank today that never passed through this
...
export default async function AccountsPage() {
  ...
  /*
    ...
    today's rate. Five of the six accounts hold kwacha (lib/accounts.ts)
  ...
  */
  const held = (row: (typeof rows)[number]) =>
    rate === null
      ? row.netUsd
      : row.account.currency === "ZMW"
        ? row.net
        : row.net * rate;

  // Accounts holding money, biggest first; the empty ones are listed after,
  // compactly. Six equal cards where four said "K 0 · no movement yet" gave
  // two-thirds of the page to nothing and made the account holding K 357,615
  // look no more important than one holding nothing at all.
  // Every account, always — an empty mobile-money till is a different fact from no
  // mobile-money till, and this page has to be able to say both. Funded first so the
  ...
  return (
    <>
      ...
      {/* The position, in kwacha, as one band rather than four flat cells.
          It was a hairline strip of dollar figures on a page whose every other
          number had moved to kwacha. */}
        ...
        >
          ...
          <span className="text-right">
            <span className="block font-display text-lg font-bold tabular-nums">
              {rate
                ? `K ${Math.round(unattributedUsd * rate).toLocaleString("en-US")}`
