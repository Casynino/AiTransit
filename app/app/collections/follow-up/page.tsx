...
import { currentRate, formatLocal, formatKwacha, formatUsd } from "@/lib/fx";
import { formatDate, toNumber } from "@/lib/format";
import { collectionsOverview } from "@/lib/collections";
import { t } from "@/lib/i18n";
import { paymentReminder, whatsappLink } from "@/lib/messages";
...
export default async function FollowUpPage({
   ...
   * is the exact one.
   *
   * Through formatKwacha rather than the `rate ? … : …` this page used to
   * carry, which wrote the amount as "ZMW 202,500" — the stored ISO code, not
   * the "K" the ledger and every dashboard print. One list quoting a currency
  ...
  const money = (usd: number) => formatKwacha(usd, liveRate);
  /**
   * The exact dollar figure, printed only where the line above it is a
   * conversion. With no rate published formatKwacha prints dollars itself,
  ...
  const creditMessage = (row: FollowUpRow) => {
    ...
    // Dated in English regardless of who is at the keyboard: the message goes
    // to a Zambian customer, and a clerk reading the app in Chinese must not
    // send them 2026年8月17日.
              ...
              >
                ...
                <td className="hidden p-3 lg:table-cell">
                  ...
                  {/*
                    ...
                    This badge carried a running kwacha figure, which put a
                  ...
                  {row.outstanding === null ? (
                    ...
                    <>
                      {/* What the customer will actually send, first — at the
                          ...
                          so it says "K" like the rest of the app; spelled out
                          here it said "ZMW", which made the same amount look
                      ...
                      <a
                        href={whatsappLink(
                          ...
                          row.credit
                            ...
                            : row.invoiceId
                              ? invoiceMessage(row)
                              : `Hello ${row.customerName.split(" ")[0]}, about your cargo ${row.trackingNumber}.`
                        )}
                        target="_blank"
